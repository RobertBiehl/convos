...
FROM current_file_edits fe JOIN provenance.file_edit_evidence v ON v.file_edit_id=fe.id AND v.status='confirmed' LEFT JOIN messages m ON fe.message_id = m.id LEFT JOIN conversations c ON c.id = m.conversation_id
WHERE fe.file_path = ? ORDER BY fe.created_at, fe.id"""
_GQ = """SELECT fe.file_path, m.conversation_id, fe.message_id, c.source FROM current_file_edits fe JOIN provenance.file_edit_evidence v ON v.file_edit_id=fe.id AND v.status='confirmed'
