...
def verify_row_proof_header(value,cert,root_public):
    try:
        ...
        if set(value)!=ROW_PROOF_FIELDS or value["v"]!=1 or value["kind"]!="row.proof" or any(not isinstance(value[k],str) or not value[k] for k in ("workspace","authorization_workspace","row_id")) or value["row_kind"] not in SEMANTIC_FIELDS_V1 or value["encoding_v"]!=1 or not hex64(value["content_hash"]) or value["state"] not in ("active","deleted") or value["revision"]!=digest({"v":1,**claim}) or previous is not None and (not hex64(previous) or previous==value["revision"]) or not isinstance(value["authorization_epoch"],int) or isinstance(value["authorization_epoch"],bool) or value["authorization_epoch"]<1 or (cert["user"],device["id"],public_id(root_public),public_id(device["sign_public"]))!=(value["author_user_id"],value["author_device_id"],value["author_user_id"],value["author_device_id"]): raise ValueError
...
def verify_row_proof(value,row,cert,root_public):
    try:
        ...
        if set(row)!={"v","kind","id","state","data"} or row["kind"] not in SEMANTIC_FIELDS_V1 or not isinstance(row["id"],str) or not row["id"] or row["v"]!=1 or row["state"] not in ("active","deleted") or row["data"] is not None and (row["state"]!="active" or set(row["data"]) not in (set(SEMANTIC_FIELDS_V1[row["kind"]]),set(SEMANTIC_FIELDS_V1[row["kind"]])-({'edits'} if row["kind"] in ('messages','tool_calls') else set()))) or row["state"]=="deleted" and row["data"] is not None or (value["row_kind"],value["row_id"],value["encoding_v"],value["content_hash"],value["state"])!=(row["kind"],row["id"],row["v"],digest(row),row["state"]): raise ValueError
        return value
