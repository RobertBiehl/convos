...
def project(db_path,state,value,workspace,local_device=None,db=None,root=None,batch=False,authors=None,recover=None,local_user=None):
    ...
    if value["kind"]=="workspace.preference":
        ...
        if set(p)!={"row","proof"} or set(row)!={"v","kind","id","state","data"} or row["v"]!=1 or row["kind"]!="sharing.preference" or row["id"]!=f"sharing:{workspace}:{user}" or row["state"]!="active" or set(data)!={"auto_contribute","match"} or data["auto_contribute"] is not None and not isinstance(data["auto_contribute"],bool) or not isinstance(match,list) or match!=[m for m in ("cwd","edit") if m in match] or proof["workspace"]!=workspace: raise ValueError("invalid sharing preference")
        ...
        state.execute("DELETE FROM team_scopes WHERE workspace=?",(workspace,))
        batch or state.commit()
        return True
