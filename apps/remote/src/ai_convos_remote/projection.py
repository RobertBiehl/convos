...
def sharing_object(state,workspace,row,proof,authors):
    ...
    if row["kind"]=="sharing.preference":
        ...
        if old and json.loads(old[0])!=evidence: raise ValueError("repository policy evidence conflict")
...
def _team_scope(core,provenance,repositories,roots,candidates=None,match=("cwd","edit")):
    ...
    return ({cid for eid,path,cid,cwd in rows if edit_repos.get(eid) in repositories or _under(path,cwd,roots)} if "edit" in match else set())|({cid for cid,cwd,repo in cwd_rows if repo in repositories or cwd and any(Path(cwd).is_relative_to(root) for root in roots)} if "cwd" in match else set())
