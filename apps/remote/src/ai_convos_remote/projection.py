...
def _team_scope(core,provenance,repositories,roots,candidates=None,match=("cwd","edit")):
    roots=[Path(p).expanduser().resolve() for p in roots]; edit_repos={r["payload"]["id"]:r["payload"]["repository"] for r in provenance if r["kind"]=="edit.observed"}; where=" WHERE c.id IN (SELECT UNNEST(?))" if candidates is not None else ""; args=[list(candidates)] if candidates is not None else []; rows=core.execute("SELECT fe.id,fe.file_path,m.conversation_id,c.cwd FROM file_edits fe JOIN messages m ON m.id=fe.message_id JOIN conversations c ON c.id=m.conversation_id"+where,args).fetchall(); cwd_rows=core.execute("SELECT c.id,s.cwd,s.repository FROM conversations c JOIN provenance.conversation_scopes s ON s.conversation=c.id"+(" WHERE c.id IN (SELECT UNNEST(?))" if candidates is not None else ""),args).fetchall()
    return ({cid for eid,path,cid,cwd in rows if edit_repos.get(eid) in repositories or _under(path,cwd,roots)} if "edit" in match else set())|({cid for cid,cwd,repo in cwd_rows if repo in repositories or roots and _under(cwd,None,roots)} if "cwd" in match else set())
...
def project(db_path,state,value,workspace,local_device=None,db=None,root=None,batch=False,authors=None,recover=None,local_user=None):
    ...
    if value["kind"]=="workspace.policy":
        p=value["payload"]
        evidence=p["evidence"] if value["payload_v"]==2 else None
        if value["payload_v"]==2 and (set(p)!={"kind","value","evidence"} or p["kind"]!="repository" or not isinstance(p["value"],str) or set(evidence)!={"lineage","remotes"} or evidence["lineage"] is not None and not isinstance(evidence["lineage"],str) or not isinstance(evidence["remotes"],list) or not all(isinstance(r,str) for r in evidence["remotes"])): raise ValueError("invalid repository policy")
        state.execute("INSERT OR REPLACE INTO policies VALUES (?,?,?,?,?)",(workspace,author_user(value,authors),p["kind"],p["value"],json.dumps(evidence) if evidence else None))
