...
from ai_convos.cli import ARCHIVE_COLUMNS as COLUMNS, ARCHIVE_FKS as FKS, PROVENANCE_KINDS as PROVENANCE, _insert_pages, _migration_backup, _transaction, archive_relationships, archive_yield, edit_parents, claim_row_owners, captured_edit_paths, gen_id, index_attachment_body, init_schema, matching_logical_row, open_db, operation_lock, preserve_fact_heads, project_attested_rows, project_edit_dependencies, project_logical_rows, project_provenance, project_provider_bindings, project_row_proofs, project_workspace_controls, provider_session_key, provenance_records, record_local_row_bases, required, retire_row_bodies, set_attachment_path, typed_logical_rows, wake_hooks
...
STATE_VERSION,PROJECTION_VERSION="4",17
REPLICA_VERSIONS=(17,)  # Source ownership and parent-owned edit metadata share the reset boundary.
STATE = """
...
TABLES={"conversation.record":"conversations","message.record":"messages","tool.record":"tool_calls","attachment.record":"attachments","artifact.record":"artifacts"}
...
def _records(core,state,blobs=True,changes=None):
    ...
    for kind,table in TABLES.items():
        ...
        if table in ('messages','tool_calls'):
            from ai_convos.cli import edit_metadata
            edits=edit_metadata(core,table,[r['payload']['row'][0] for r in records])
            records=[{**r,'payload':{**r['payload'],'columns':[*cols,'edits'],'row':[*r['payload']['row'],edits.get(r['payload']['row'][0],[])]}} for r in records]
...
def scan(core,graph,kind="personal",repositories=(),roots=(),changes=None,workspace=None,new_scope=None,match=("cwd","edit"),user=None,selected=None):
    local={tuple(r) for r in core.execute("SELECT kind,entity FROM provenance.local_facts"+(f" WHERE entity IN ({TEXT_IDS})" if facts is not None else ""),[packed(r[1] for r in facts)] if facts is not None else []).fetchall()} if (facts:=None if changes is None else {(k.removeprefix("retired."),entity) for k,entity in changes if k.removeprefix("retired.") in PROVENANCE}) is None or facts else set()
    all_provenance=[clean(r) for r in provenance_records(core,facts) if (r["kind"],r["entity"]) in local] if local else []
    changed={table:{entity for name,entity in changes or () if name==table} for table in COLUMNS}
    ...
    changes=changes|set(edit_parents(core,[entity for name,entity in changes if name in ('file_edits','edit.observed')])) if changes is not None else None
    ...
    retired=core.execute("SELECT DISTINCT p.row_kind,p.source_row_id,p.authorization_workspace_id FROM remote.row_proofs p JOIN archive_sync s ON (s.user_id,s.device_id)=(p.author_user_id,p.author_device_id) JOIN archive_changes a ON (a.kind,a.entity)=('retired.'||p.row_kind,p.source_row_id) WHERE p.row_kind IN (SELECT UNNEST(?)) AND p.state='active' AND NOT EXISTS (SELECT 1 FROM provenance.local_facts l WHERE (l.kind,l.entity)=(p.row_kind,p.source_row_id)) AND NOT EXISTS (SELECT 1 FROM remote.row_proofs c WHERE (c.row_kind,c.source_row_id,c.author_user_id,c.previous_revision)=(p.row_kind,p.source_row_id,p.author_user_id,p.revision))"+(" AND p.source_row_id IN (SELECT UNNEST(?))" if facts is not None else ""),[list(PROVENANCE)]+([[entity for k,entity in facts]] if facts is not None else [])).fetchall() if facts is None or facts else []
    tombstones=[dict(kind=k,entity=entity,payload=dict(id=entity,state="deleted"),observed_at=None) for k,entity in sorted({(k,entity) for k,entity,scope in retired if kind=="personal" or scope==workspace})]
    if kind=="personal" or selected is not None: return records+provenance+tombstones
    keep=[]
...
def scan_archive(db_path,graph,kind="personal",repositories=(),roots=(),workspace=None,new_scope=None,match=("cwd","edit"),user=None,generation=None,progress=None,page=2500,since=None):
    ...
    if since is None: sources.append(("SELECT a.kind,a.entity FROM archive_changes a WHERE starts_with(a.kind,'retired.') AND a.generation<=? AND NOT EXISTS (SELECT 1 FROM provenance.local_facts l WHERE (l.kind,l.entity)=(substr(a.kind,9),a.entity))",(generation,)))
...
def retained_proof_pages(db_path,workspace,origins=(),author=None,page=5000):
    ...
    while True:
        with contextlib.closing(open_db(db_path,True,purpose="remote.replicas.page")) as db: rows=db.execute(sql,(*scopes,author,[*TABLES.values(),*PROVENANCE],*scopes,*scopes,*scopes,after,page)).fetchall()
        if not rows: return
...
def verified_replica(body,workspace,controls,user):
    ...
    if lineage is not None:
        ...
        for parent in lineage:
            ...
            if row["kind"] in COLUMNS and parent["author_device_id"]!=proof["author_device_id"]: raise ValueError("Source device ownership conflict in revision lineage")
...
def apply_row_replicas(db_path,bodies,workspace,controls,recover=None,local_user=None,db=None,root=None,ready=True,local_device=None,retry=False):
    ...
    try:
        with _transaction(db):
            ...
            claim_row_owners(db,[(row,p) for row,p,signer_,lineage in items])
            ...
            for scope,nodes in chains.items():
                ...
                if leaves and len({nodes[r][2]['content_hash'] for r in leaves})==1:
                    ...
                    if row is not None: projected.append((row,p,pid,p["author_user_id"]==local_user and (local_device is None or p["author_device_id"]==local_device)))
            pending=set()
            own_ids={(kind,entity):entity for row,p,pid,native in projected if p['author_user_id']==local_user for kind,entity in [(row['kind'],row['id']),*[(parent,row['data'][column]) for column,parent in FKS.get(row['kind'],()) if row['state']=='active' and row['data'][column]],*[(kind,entity) for e in (row['data'] or {}).get('edits',[]) for kind,entity in [('file_edits',e['id']),('messages',e['message_id'])]],*([('file_edits',row['id']),('messages',row['data']['turn'])] if row['kind']=='edit.observed' and row['state']=='active' else [('file_edits',row['data']['edit'])] if row['kind']=='checkpoint.link' and row['state']=='active' else [])]}
            ...
            again=project_edit_dependencies(db,waiting,[dependency(kind,entity,p['author_user_id']) for row,p,pid,native in projected if pid not in pending for kind,entity in ([(row['kind'],row['id'])] if row['kind'] in ('file_edits','file.observed') else [('file_edits',e['id']) for e in (row['data'] or {}).get('edits',[])])],[pid for pid,key in work]+[digest(p) for row,p,signer_,lineage in items if row['kind']=='edit.observed'],advanced)
