...
def atomic_json(path,data):
    ...
    tmp.touch(mode=mode,exist_ok=False)
    os.chmod(tmp,mode)
    tmp.write_text(json.dumps(data,indent=2)+"\n")
    os.replace(tmp,path)
def _db_path(root=None):
    if root is not None: return Path(root).expanduser()/"memory"/"state.db"
    base=Path(os.environ.get("CONVOS_PROJECT_ROOT",Path.home()/".convos")).expanduser()
    return Path(os.environ.get("CONVOS_MEMORY_DB",base/"memory"/"state.db")).expanduser()
def _remote(value):
    ...
    return host.lower() + (slash+path if slash else "")
def _git(root, *args):
...
def _repository(root):
    ...
    try:
        ...
        lineage = "lineage_"+_hash("\0".join(roots))[:20] if roots else None
        return dict(repository="repo_"+_hash("remote\0"+origin)[:20] if origin else lineage, lineage=lineage, checkout=str(root)) if origin or lineage else None
    except (OSError, ValueError): return None
def _raw_scope(scope=None):
    path = Path(scope or Path.cwd()).expanduser()
    missing = bool(scope) and not path.exists()
    path = path if missing else path.resolve()
    return scope if missing else str(next((p for p in (path, *path.parents) if (p/".git").exists()), path))
def _scope(scope=None):
    raw = _raw_scope(scope)
    repo, path = _repository(raw), _db_path()
    if not repo or path.is_symlink() or not path.is_file(): return raw
    db = sqlite3.connect(path)
    if db.execute("PRAGMA user_version").fetchone()[0] in (0,1,2,3,4):
        db.close()
        with contextlib.closing(connect()): pass
        return _scope(raw)
    try:
        ...
        if rows:
            db.execute("INSERT OR REPLACE INTO repository_scopes VALUES (?,?,?,?,?)",(repo["repository"],repo["lineage"],rows[0][0],repo["checkout"],datetime.now(timezone.utc).isoformat()))
            db.commit()
...
def _bind_scope(db, scope):
    ...
    db.execute("INSERT OR REPLACE INTO repository_scopes VALUES (?,?,?,?,?)", (repo["repository"],repo["lineage"],chosen,repo["checkout"],datetime.now(timezone.utc).isoformat()))
    return chosen
def connect(root=None):
    ...
    if (version := db.execute("PRAGMA user_version").fetchone()[0]) not in (0,1,2,3,4,5,6):
        db.close()
        raise ValueError("Unsupported memory ledger schema version; expected 0 through 6")
    db.executescript(SCHEMA)
    if version<6: db.executescript("DROP TABLE IF EXISTS remote_heads;DROP TABLE IF EXISTS remote_parts;")
    db.execute("INSERT OR IGNORE INTO remote_meta VALUES ('ledger_id',?)",(os.urandom(16).hex(),))
    db.commit()
    if version < 2:
        values=[(repo["repository"],repo["lineage"],scope,repo["checkout"],datetime.now(timezone.utc).isoformat()) for scope, in db.execute("SELECT scope FROM sources UNION SELECT scope FROM canonicals") if (repo:=_repository(scope))]
        db.executemany("INSERT OR IGNORE INTO repository_scopes VALUES (?,?,?,?,?)",values)
        db.commit()
    if db.execute("SELECT 1 FROM links l JOIN sources s ON s.id=l.source JOIN canonicals c ON c.id=l.canonical WHERE s.scope<>c.scope LIMIT 1").fetchone():
        db.close()
        raise ValueError("Cross-scope memory link in ledger")
    return db
...
def _codex():
    root = Path(os.environ.get("CONVOS_CODEX_MEMORY_ROOT", Path(os.environ.get("CODEX_HOME", Path.home()/".codex"))/"memories")).expanduser()
    path = root/"MEMORY.md"
    if not root.exists(): return [], set()
    if not path.exists(): return [], {"codex"}
    text = path.read_text()
    parts = [p.strip() for p in re.split(r"(?m)(?=^# Task Group: )", text) if p.lstrip().startswith("# Task Group:")]
...
def _claude_history():
    path = _claude_root().parent/"history.jsonl"
...
def _claude_scope(project, scope=None, history=None):
    ...
    indexed = json.loads(index.read_text())["entries"] if index.exists() else []
    cwds = {_scope(cwd) for path in project.glob("*.jsonl") if (cwd := _session_cwd(path))} | {_scope(e["projectPath"]) for e in indexed}
    origins = {m.group(1) for path in (project/"memory").glob("*.md") if (m := re.search(r"(?m)^originSessionId:\s*(\S+)", path.read_text()))}
    history = _claude_history() if history is None else history
    cwds |= {_scope(cwd) for origin in origins for cwd in history.get(origin, ())}
    if len(cwds) > 1:
...
def _claude(scope=None):
    root = _claude_root()
    if not root.exists(): return [], set()
    files = sorted(p for p in root.glob("*/memory/*.md") if p.name not in ("MEMORY.md", "convos-synced.md"))
    history = _claude_history()
    scopes = {p:_claude_scope(p, scope, history) for p in {f.parent.parent for f in files}}
...
def discover(scope=None):
    ci, ca = _codex()
    hi, ha = _claude(scope)
    return [i for i in ci + hi if scope is None or i["scope"] == scope], ca | ha
...
def scan_store(scope=None):
    ...
    items, available = discover(scope)
    db, now, revisions = connect(), datetime.now(timezone.utc).isoformat(), 0
    with db:
        scope = _bind_scope(db,scope) if scope is not None else None
        items = list({(item["provider"],bound := _bind_scope(db,item["scope"]),item["locator"]):{**item,"scope":bound} for item in items}.values())
    ...
    counts = dict(db.execute("SELECT COUNT(*),SUM(active),SUM(NOT active) FROM sources WHERE ? IS NULL OR scope=?", (scope, scope)).fetchone())
    db.close()
...
def remember_data(content, scope=None, canonical=None, evidence=()):
    ...
    with contextlib.closing(db),_transaction(db):
        scope = _bind_scope(db,scope)
        replacing = canonical is not None
        cid = _select_canonical(db,scope,canonical)["id"] if replacing else "mem_" + _hash(scope + "\0" + content)[:16]
        row = db.execute("SELECT * FROM canonicals WHERE id=?", (cid,)).fetchone()
        origins = db.execute("SELECT s.* FROM links l JOIN sources s ON s.id=l.source WHERE l.canonical=? ORDER BY s.id", (cid,)).fetchall()
        ...
        locator, digest = f"user/{cid}", _hash(content)
        sid = origins[0]["id"] if replacing else _hash("\0".join(("user",scope,locator)))[:20]
        prior = db.execute("SELECT content FROM sources WHERE id=?", (sid,)).fetchone()
        status = "unchanged" if prior and prior["content"] == content else "revised" if replacing else "linked" if row else "created"
        db.execute("INSERT INTO sources VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET hash=excluded.hash,content=excluded.content,active=1,observed_at=excluded.observed_at", (sid,"user",scope,locator,str(_db_path()),digest,content,1,now))
        db.execute("INSERT OR IGNORE INTO revisions VALUES (?,?,?,?)", (sid,digest,content,now))
        if not row or row["content"] != content: _canonical(db,cid,scope,content,now)
        db.execute("INSERT INTO links VALUES (?,?,?) ON CONFLICT(source) DO UPDATE SET canonical=excluded.canonical,applied_hash=excluded.applied_hash", (sid,cid,digest))
        [db.execute("INSERT OR IGNORE INTO evidence VALUES (?,?,?,?,?,?,?,?,?)",(cid,digest,*(p[k] for k in ("message","conversation","source","title","role","created_at","content_hash")))) for p in proofs]
...
def forget_data(canonical, scope=None, dry_run=False):
    ...
    with contextlib.closing(db),_transaction(db,not dry_run):
        row = _select_canonical(db,scope,canonical)
        canonical = row["id"]
        origins = db.execute("SELECT s.id,s.provider,s.locator,s.hash,s.active,l.applied_hash FROM links l JOIN sources s ON s.id=l.source WHERE l.canonical=?", (canonical,)).fetchall()
        ...
        revisions = db.execute("SELECT COUNT(*) FROM canonical_revisions WHERE canonical=?", (canonical,)).fetchone()[0] + sum(db.execute("SELECT COUNT(*) FROM revisions WHERE source=?", (r["id"],)).fetchone()[0] for r in origins)
        evidence = db.execute("SELECT COUNT(*) FROM evidence WHERE canonical=?",(canonical,)).fetchone()[0]
        [db.execute("UPDATE remote_semantics SET owned=1 WHERE entity=?",(r["locator"].split("/",2)[-1],)) for r in origins if r["provider"]=="remote"]
        [db.execute("DELETE FROM links WHERE source=?", (r["id"],)) for r in origins]
        [db.execute("DELETE FROM revisions WHERE source=?", (r["id"],)) for r in origins]
        [db.execute("DELETE FROM sources WHERE id=?", (r["id"],)) for r in origins]
        db.execute("DELETE FROM evidence WHERE canonical=?", (canonical,))
        db.execute("DELETE FROM canonical_revisions WHERE canonical=?", (canonical,))
        db.execute("DELETE FROM canonicals WHERE id=?", (canonical,))
...
def apply_data(document, dry_run=False):
    ...
    scan_store(document.get("scope"))
    db, now, applied = connect(), datetime.now(timezone.utc).isoformat(), 0
    with contextlib.closing(db),_transaction(db,not dry_run):
        ...
        for decision in resolutions:
            ...
            if action == "revise":
                ...
                _canonical(db, decision["canonical"], row["scope"], decision["content"], now)
                applied += 1
                continue
            ...
            if action in ("distinct", "scoped"):
                ...
                cid = cid or "mem_" + _hash(scope + "\0" + content)[:16]
                _canonical(db, cid, scope, content, now)
...
def reconcile_data(scope, dry_run=False, refresh=True, retry=True):
    refresh and scan_store(scope)
    plan = plan_data(True, scope=scope)
    pending = plan["pending"]
    groups = {(r["scope"],r["hash"]):[p for p in pending if (p["scope"],p["hash"]) == (r["scope"],r["hash"]) and p["kind"] == "unlinked"] for r in pending}
...
def sync_data(scope=None):
    scope = _scope(scope)
    automatic = reconcile_data(scope)["applied"]
    plan = plan_data(True, scope=scope)
    return dict(**plan, status="clean" if not plan["pending"] else "needs_resolution", automatic=automatic, remaining=len(plan["pending"]))
def sync_all_data():
    scan_store()
    db = connect()
    scopes = [r[0] for r in db.execute("SELECT scope FROM sources UNION SELECT scope FROM canonicals ORDER BY scope")]
    db.close()
    rows = [sync_data(scope) for scope in scopes]
    concise = [{k:r[k] for k in ("scope","status","automatic","remaining")} for r in rows]
...
def review_data(scope=None):
    result = sync_data(scope)
    pending = result["pending"]
    canonicals = {r["id"]:r for r in result["canonicals"]}
    header = f"# Memory changes\n\nProject: `{result['scope']}`"
...
def status_data(scope):
    ...
    db = connect()
    args = (scope, scope)
    sources = [r[0] for r in db.execute("SELECT active FROM sources WHERE ? IS NULL OR scope=?", args)]
    ...
    revisions = db.execute("SELECT COUNT(*) FROM revisions r JOIN sources s ON s.id=r.source WHERE ? IS NULL OR s.scope=?", args).fetchone()[0]
    canonicals = db.execute("SELECT COUNT(*) FROM canonicals WHERE ? IS NULL OR scope=?", args).fetchone()[0]
    result = dict(scope=scope, sources=len(sources), active=sum(sources), missing=len(sources)-sum(sources), revisions=revisions, canonicals=canonicals, user_owned=owned(scope), pending=len(plan_data(db=db, scope=scope)["pending"]), database=str(_db_path()))
...
def _snapshot(path):
    ...
    db, expected = sqlite3.connect(path.resolve().as_uri()+"?mode=ro", uri=True), sqlite3.connect(":memory:")
    db.execute("BEGIN")
    expected.executescript(SCHEMA)
    tables = {r[0]:r[1] for r in expected.execute("SELECT name,sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")}
    expected.close()
    try:
        version=db.execute("PRAGMA user_version").fetchone()[0]
        base=("sources","revisions","canonicals","canonical_revisions","links","projections")
        ...
        if version<6:
            ...
            db.close()
            db=migrated
            db.executescript(SCHEMA+"DROP TABLE IF EXISTS remote_heads;DROP TABLE IF EXISTS remote_parts;")
            db.execute("INSERT OR IGNORE INTO remote_meta VALUES ('ledger_id',?)",(os.urandom(16).hex(),))
            db.commit()
...
def backup_data(target=None):
    source = connect()
    path = Path(target).expanduser() if target else _db_path().parent/"backups"/f"state-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}.db"
    if path.is_symlink() or path.exists():
        source.close()
        raise ValueError(f"Memory backup target already exists or is a symlink: {path}")
    path.parent.mkdir(parents=True, exist_ok=True)
...
def restore_data(source, yes=False):
    ...
    if not yes:
        snapshot.close()
        return dict(status="would_restore",path=str(source.resolve()),**summary)
    rescue = backup_data()
    live = connect()
    try:
...
def adopt_scope_data(scope, checkout=None, yes=False):
    ...
    db = connect()
    canonicals = db.execute("SELECT COUNT(*) FROM canonicals WHERE scope=?", (scope,)).fetchone()[0]
    sources = db.execute("SELECT COUNT(*) FROM sources WHERE scope=?", (scope,)).fetchone()[0]
    if not canonicals and not sources:
        db.close()
        raise ValueError(f"Memory scope does not exist: {scope}")
    if yes:
        db.execute("INSERT OR REPLACE INTO repository_scopes VALUES (?,?,?,?,?)",(repo["repository"],repo["lineage"],scope,repo["checkout"],datetime.now(timezone.utc).isoformat()))
        db.commit()
    db.close()
    return dict(status="adopted" if yes else "would_adopt",scope=scope,checkout=repo["checkout"],repository=repo["repository"],canonicals=canonicals,sources=sources)
def remote_records(root,user,workspace,kind):
    if kind!="personal": return []
    db=connect(root)
    stored=[dict(row=json.loads(r["body"]),proof=json.loads(r["proof"]),previous=None,owned=bool(r["owned"])) for r in db.execute("SELECT * FROM remote_semantics ORDER BY workspace,author,entity,revision")]
    rows=[dict(r) for r in db.execute("""SELECT c.*,EXISTS(SELECT 1 FROM links l JOIN sources s ON s.id=l.source WHERE l.canonical=c.id AND s.provider<>'remote') local_origin,EXISTS(SELECT 1 FROM links l JOIN sources s ON s.id=l.source WHERE l.canonical=c.id AND s.provider='remote' AND s.hash=c.hash) remote_exact,EXISTS(SELECT 1 FROM links l JOIN sources s ON s.id=l.source WHERE l.canonical=c.id AND (NOT s.active OR s.hash<>l.applied_hash)) pending FROM canonicals c ORDER BY c.id""")]
    repositories={r["scope"]:dict(x) for r in rows if (x:=db.execute("SELECT repository,lineage FROM repository_scopes WHERE scope=? ORDER BY checkout LIKE 'remote:%',checkout LIMIT 1",(r["scope"],)).fetchone())}
    records,seen=stored.copy(),set()
    for value in rows:
        repo=repositories.get(value["scope"])
        scope=repo["repository"] if repo else "global" if value["scope"]=="global" else "scope_"+_hash(value["scope"])[:20]
        entity=f"memory:{scope}:{value['id']}"
        matching=[r for r in stored if r["proof"]["author_user_id"]==user and r["row"]["id"]==entity]
        imported=any(r["row"]["state"]=="active" and r["row"]["data"]["hash"]==value["hash"] and not r["owned"] for r in stored)
        local=value["local_origin"] or not value["remote_exact"] or not imported
        if not local: continue
        seen.add(entity)
        row={"v":1,"kind":"memory.canonical","id":entity,"state":"active","data":{"canonical":value["id"],"scope":scope,"repository":repo["repository"] if repo else None,"lineage":repo["lineage"] if repo else None,"hash":value["hash"],"content":value["content"],"updated_at":value["updated_at"]}}
...
def _remote_apply(db,root,workspace,author,entity,p,content,observed_at,conflict=False):
    locator=f"{workspace}/{author}/{entity}"
    found=db.execute("SELECT id,scope FROM sources WHERE provider='remote' AND locator=?",(locator,)).fetchone()
    if content is None:
        if not found: return
        ...
        sid=found["id"] if found else _hash("\0".join(("remote",target,locator)))[:20]
        target=found["scope"] if found else target
    if content is not None:
        digest=p["hash"]
        link=db.execute("SELECT l.canonical,l.applied_hash,c.hash FROM links l JOIN canonicals c ON c.id=l.canonical WHERE l.source=?",(sid,)).fetchone()
        db.execute("""INSERT INTO sources VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET hash=excluded.hash,content=excluded.content,active=1,observed_at=excluded.observed_at""",(sid,"remote",target,locator,str(_db_path(root)),digest,content,1,observed_at))
        db.execute("INSERT OR IGNORE INTO revisions VALUES (?,?,?,?)",(sid,digest,content,observed_at))
        if conflict: return
        ...
        if link and link["hash"]==link["applied_hash"] and not blocked:
            _canonical(db,link["canonical"],target,content,observed_at)
            db.execute("UPDATE links SET applied_hash=? WHERE source=?",(digest,sid))
        ...
        elif not link and not db.execute("SELECT 1 FROM canonicals WHERE scope=? LIMIT 1",(target,)).fetchone():
            cid="mem_"+_hash(target+"\0"+content)[:16]
            _canonical(db,cid,target,content,observed_at)
            db.execute("INSERT INTO links VALUES (?,?,?)",(sid,cid,digest))
    elif found:
        if conflict:
            db.execute("DELETE FROM revisions WHERE source=?",(sid,))
            db.execute("UPDATE sources SET content='',active=0,observed_at=? WHERE id=?",(observed_at,sid))
            return
        link=db.execute("SELECT l.canonical,l.applied_hash,c.hash FROM links l JOIN canonicals c ON c.id=l.canonical WHERE l.source=?",(sid,)).fetchone()
        protected=link and (db.execute("SELECT 1 FROM links WHERE canonical=? AND source<>? LIMIT 1",(link["canonical"],sid)).fetchone() or any((path:=Path(r[0])).exists() and (path.is_symlink() or f"canonical:{link['canonical']} " in path.read_text()) for r in db.execute("SELECT target FROM projections")))
        if link and link["hash"]==link["applied_hash"] and not protected:
            db.execute("DELETE FROM links WHERE source=?",(sid,))
            db.execute("DELETE FROM revisions WHERE source=?",(sid,))
            db.execute("DELETE FROM sources WHERE id=?",(sid,))
            db.execute("DELETE FROM canonical_revisions WHERE canonical=?",(link["canonical"],))
...
def remote_accept(root,row,proof,project=True):
    ...
    fields={"canonical","scope","repository","lineage","hash","content","updated_at"}
    ...
    db=connect(root)
    author,entity=proof["author_user_id"],row["id"]
    with contextlib.closing(db),_transaction(db):
        old=db.execute("SELECT owned FROM remote_semantics WHERE workspace=? AND author=? AND entity=? AND revision=?",(proof["workspace"],author,entity,proof["revision"])).fetchone()
        owned=bool(old and old["owned"])
        protected=owned or bool(db.execute("SELECT 1 FROM remote_semantics WHERE workspace=? AND author=? AND entity=? AND owned",(proof["workspace"],author,entity)).fetchone())
        db.execute("INSERT OR REPLACE INTO remote_semantics VALUES (?,?,?,?,?,?,?,?)",(proof["workspace"],author,entity,proof["revision"],row["state"],json.dumps(row,sort_keys=True,separators=(",",":")),json.dumps(proof,sort_keys=True,separators=(",",":")),int(owned or not project)))
        values=[dict(row=json.loads(r["body"]),proof=json.loads(r["proof"])) for r in db.execute("SELECT body,proof FROM remote_semantics WHERE workspace=? AND author=? AND entity=?",(proof["workspace"],author,entity))]
        ancestors={a for value in values for a in value["proof"]["ancestors"]}
        leaves=[value for value in values if value["proof"]["revision"] not in ancestors]
        db.execute("DELETE FROM remote_semantics WHERE workspace=? AND author=? AND entity=? AND revision NOT IN (%s)"%",".join("?"*len(leaves)),(proof["workspace"],author,entity,*(v["proof"]["revision"] for v in leaves)))
        if project and not protected:
            current=(db.execute("SELECT c.hash FROM sources s JOIN links l ON l.source=s.id JOIN canonicals c ON c.id=l.canonical WHERE s.provider='remote' AND s.locator=?",(f"{proof['workspace']}/{author}/{entity}",)).fetchone() or [None])[0]
            deleted=next((v for v in leaves if v["row"]["state"]=="deleted"),None)
            chosen=leaves if len(leaves)==1 else [deleted or next((v for v in leaves if v["row"]["data"]["hash"]!=current),leaves[0])]
            for value in chosen:
                body,p=value["row"],value["proof"]
                payload=body["data"]
                _remote_apply(db,root,proof["workspace"],p["author_user_id"],entity,payload,payload["content"],payload["updated_at"],len(leaves)>1) if body["state"]=="active" else _remote_apply(db,root,proof["workspace"],p["author_user_id"],entity,{},None,datetime.now(timezone.utc).isoformat(),len(leaves)>1)
    return len(leaves)==1
def remote_bridge(): return dict(v=3,schema=1,objects={"memory.canonical"},records=remote_records,accept=remote_accept)
def _skill_source():
    rel = Path("skills")/"convos"/"SKILL.md"
    root = Path(os.environ.get("CONVOS_PROJECT_ROOT", Path.home()/".convos")).expanduser()
...
def _hook_valid(command, path):
    ...
    return len(parts) == 4 and parts[0] == f"CONVOS_MEMORY_DB={path}" and Path(parts[1]).is_file() and os.access(parts[1], os.X_OK) and parts[2:] == ["memory", "runtime-hook"]
...
def _codex_hook_trust(command, cwd):
    ...
    process = None
    try:
        process = subprocess.Popen([binary,"app-server","--stdio"], cwd=cwd, text=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
        output = queue.Queue()
        threading.Thread(target=_rpc_reader,args=(process.stdout,output),daemon=True).start()
...
def _health_data():
    scope, path = _scope(), _db_path().resolve()
    ...
    commands = [(source,h["command"]) for source,config in _hook_paths() for g in _hook_data(source, config)[1].get("SessionStart", []) for h in g.get("hooks", []) if _hook_valid(h.get("command", ""), path)]
    hooks = len(commands)
    codex_command = next((c for s,c in commands if s == "codex"), None)
    trust = _codex_hook_trust(codex_command, scope)
    expected = _hash(skill.read_text())
    skills = sum(p.exists() and _hash(p.read_text()) == expected for root in (codex, claude) if (p := root/"skills"/"convos"/"SKILL.md"))
    ...
    sources, active = db.execute("SELECT COUNT(*),COALESCE(SUM(active),0) FROM sources WHERE scope=?", (scope,)).fetchone()
    canonicals = db.execute("SELECT COUNT(*) FROM canonicals WHERE scope=?", (scope,)).fetchone()[0]
    pending = db.execute("""SELECT COUNT(*) FROM sources s LEFT JOIN links l ON l.source=s.id WHERE ((s.active AND (l.source IS NULL OR l.applied_hash<>s.hash)) OR (NOT s.active AND l.source IS NOT NULL)) AND s.scope=?""", (scope,)).fetchone()[0]
    db.close()
    trusted = trust in ("trusted","managed")
    trust_repair = "install Codex to verify memory delivery" if trust == "unavailable" else "review Codex hooks with /hooks" if trust in ("untrusted","modified") else "verify Codex hook loading with /hooks"
    ready = hooks == 2 and skills == 2 and trusted and not pending
    repairs = ([] if hooks == 2 and skills == 2 else ["convos memory enable"]) + ([] if trusted or not codex_command else [trust_repair]) + ([] if not pending else ['ask Codex or Claude: "sync my memories"'])
...
def doctor_status():
    try:
        r=_health_data()
        detail="ledger=missing" if not r["ledger"] else f"sources={r['sources']}, active={r['active']}, canonicals={r['canonicals']}, pending={r['pending']}"
...
def _render_home(r):
    ...
    count=f"{r['canonicals']} {'memory' if r['canonicals']==1 else 'memories'} available"
    first=f"Memory {'is ready' if r['ready'] else 'needs attention'} for `{r['scope']}`: {count}."
    next_=lambda x:f"Run `{x}`." if x.startswith("convos ") else x[0].upper()+x[1:]+"."
...
def audit_data(scope=None, message=None):
    db=connect()
    matches=db.execute("SELECT DISTINCT e.message FROM evidence e JOIN canonicals c ON c.id=e.canonical WHERE (? IS NULL OR c.scope=?) AND substr(e.message,1,length(?))=? LIMIT 2",(scope,scope,message,message)).fetchall() if message else []
    if message and len(matches)!=1:
        db.close()
        raise ValueError("memory evidence message is missing or ambiguous")
    message=matches[0][0] if message else None
    groups=db.execute("SELECT DISTINCT e.canonical,e.hash,c.scope,e.hash=c.hash current FROM evidence e JOIN canonicals c ON c.id=e.canonical WHERE (? IS NULL OR c.scope=?) AND (? IS NULL OR e.message=?) ORDER BY c.scope,e.canonical,e.hash",(scope,scope,message,message)).fetchall()
    rows=[{**e,"canonical":g["canonical"],"hash":g["hash"],"scope":g["scope"],"current":bool(g["current"])} for g in groups for e in _evidence(db,g["canonical"],g["hash"]) if message is None or e["message"]==message]
    db.close()
    counts={s:sum(r["status"]==s for r in rows) for s in ("verified","changed","missing","unavailable")}
    return dict(scope=scope,message=message,evidence=len(rows),**counts,records=rows)
def _render_audit(result):
    rows=result["records"] if result["message"] else [r for r in result["records"] if r["status"]!="verified"]
    summary=f"# Memory evidence audit\n\nScope: `{result['scope'] or 'all scopes'}`  \nEvidence: {result['evidence']}  \nVerified: {result['verified']}  \nChanged: {result['changed']}  \nMissing: {result['missing']}  \nUnavailable: {result['unavailable']}"
...
def _context_label(content):
    ...
    return (next((line.split(":",1)[1].strip().strip("\"'") for line in lines[:12] if line.startswith("name:")), None) or next((line.lstrip("# ").strip() for line in lines if line.startswith("#")), None) or next((line.strip() for line in lines if line.strip()), "memory"))[:96]
def _select_canonical(db, scope, selector):
    rows=[dict(r) for r in db.execute("SELECT * FROM canonicals WHERE scope=? ORDER BY id",(scope,))]
    key=selector.strip().casefold()
    tiers=([r for r in rows if r["id"]==selector],[r for r in rows if key and r["id"].startswith(selector)],[r for r in rows if r["content"].strip().casefold()==key],[r for r in rows if _context_label(r["content"]).casefold()==key],[r for r in rows if key and key in r["content"].casefold()])
    matches=next((m for m in tiers if m),[])
...
def _render_index(rows, scope, size):
    shown = rows[:64]
    command = f"convos memory context --scope {shlex.quote(scope)}"
...
def live_context(scope=None, query=None, payload=None, limit=None, index=False):
    scope = scope or runtime_scope(payload)
    scan_store(scope)
    return context_data(scope, query, reconcile_data(scope, refresh=False)["remaining"], limit, index)
def runtime_scope(payload):
    ...
    row = db.execute("""SELECT m.scope FROM (SELECT c.scope,s.provider,s.locator FROM canonicals c LEFT JOIN links l ON l.canonical=c.id LEFT JOIN sources s ON s.id=l.source
        ...
        GROUP BY m.scope ORDER BY m.scope=? DESC,LENGTH(m.scope) DESC LIMIT 1""", (cwd, cwd, key+"/%", cwd)).fetchone()
    db.close()
    return row[0] if row else _scope(cwd)
...
def context_hook_config(remove=False, status=False):
    ...
    for source,path in _hook_paths():
        if remove and not path.exists():
            plans.append((source,path,None,0))
            continue
        ...
        if not status:
            ...
            if not remove:
                binary = Path(sys.executable).with_name("convos")
                command = f"CONVOS_MEMORY_DB={shlex.quote(str(_db_path().resolve()))} {shlex.quote(str(binary))} memory runtime-hook"
    ...
    if not status:
        for _,path,data,_ in plans:
            if data is not None: atomic_json(path,data)
    return [(source,path,count) for source,path,_,count in plans]
def enable_data(scope=None, all_=False, install=True):
    context_hook_config(status=True)
    sync = sync_all_data() if all_ else sync_data(scope)
    skills = 2
    if install:
        binary = Path(sys.executable).with_name("convos")
        result = subprocess.run([binary, "install-skills"], text=True, capture_output=True)
        skills = sum(line.startswith("Installed ") for line in result.stdout.splitlines())
    ...
    configs = context_hook_config()
    codex = next(r[1] for r in configs if r[0] == "codex")
    command = next(h["command"] for g in _hook_data("codex",codex)[1]["SessionStart"] for h in g["hooks"] if h["command"].endswith(" memory runtime-hook"))
    return dict(**sync, skills=skills, hooks=sum(r[2] for r in configs), codex_trust=_codex_hook_trust(command, str(Path.cwd())), settings=[str(r[1]) for r in configs])
...
def initialize():
    try:
        result=enable_data(install=False)
        return _enable_message(result,f"project `{result['scope']}`")
...
def projection_data(target, scope):
    scan_store(scope)
    path = _projection_path(target, scope)
    ...
    digest, existing = _hash(content), _hash(path.read_text()) if path.exists() else None
    db = connect()
    known = db.execute("SELECT hash FROM projections WHERE provider='claude-code' AND target=?", (str(path),)).fetchone()
    db.close()
...
def remove_projection(target, scope):
    ...
    db = connect()
    known = db.execute("SELECT hash FROM projections WHERE provider='claude-code' AND target=?", (str(path),)).fetchone()
    existing = _hash(path.read_text()) if path.exists() else None
    if existing is not None and not known:
        db.close()
        raise ValueError(f"Projection at {path} is not owned; refusing to remove")
    if existing is not None and existing != known[0]:
        db.close()
        raise ValueError(f"Projection drift at {path}; refusing to remove")
    if path.exists(): path.unlink()
    with db: db.execute("DELETE FROM projections WHERE provider='claude-code' AND target=?", (str(path),))
    db.close()
    return dict(provider="claude-code", target=str(path), scope=scope, status="removed" if existing else "absent")
@memory.command(hidden=True)
def scan():
    """Record exact provider revisions and mark disappeared sources."""
    _emit(_safe(scan_store))
@memory.command()
def status(scope: Optional[str] = typer.Option(None,"--project"), all_: bool = typer.Option(False, "--all"), json_output: bool = typer.Option(False, "--json")):
    ...
    if json_output:
        _emit(result)
        return
    summary=lambda r:f"{r['canonicals']} {'memory' if r['canonicals']==1 else 'memories'}, {r['user_owned']} remembered, {r['pending']} {'change needs' if r['pending']==1 else 'changes need'} review"+(f", {r['missing']} {'origin is' if r['missing']==1 else 'origins are'} unavailable" if r["missing"] else "")
    label = result["scope"] or "all projects"
    state = "ready" if not result["pending"] else "needs attention"
...
def audit(scope: Optional[str] = typer.Option(None,"--project"), all_: bool = typer.Option(False,"--all"), message: Optional[str] = typer.Option(None,"--message",help="Find the memory revisions supported by one exact archived message."), json_output: bool = typer.Option(False,"--json")):
    if all_ and scope is not None:
        typer.echo("--all cannot be combined with --project",err=True)
        raise typer.Exit(1)
    result=_safe(lambda:audit_data(None if all_ else _scope(scope),message))
    _emit(result) if json_output else typer.echo(_render_audit(result))
@memory.command()
def sync(scope: Optional[str] = typer.Option(None,"--project"), all_: bool = typer.Option(False, "--all"), json_output: bool = typer.Option(False, "--json")):
    ...
    if json_output: _emit(result)
        ...
        automatic, remaining = result["automatic"], result["remaining"]
        updated = "nothing changed" if not automatic else f"updated {automatic} {'memory' if automatic == 1 else 'memories'}"
        if not all_: typer.echo(f"Memory {'is up to date' if not remaining else 'checked'} for {result['scope']}: {updated}." + ("" if not remaining else f" {remaining} {'change needs' if remaining == 1 else 'changes need'} a decision; ask Codex or Claude: \"sync my memories\"."))
            ...
            attention = sum(bool(r["remaining"]) for r in result["scopes"])
            typer.echo(f"Memory {'is up to date' if not remaining else 'checked'} across {len(result['scopes'])} projects: {updated}." + ("" if not remaining else f" {remaining} {'change needs' if remaining == 1 else 'changes need'} a decision in {attention} {'project' if attention == 1 else 'projects'}."))
            for row in result["scopes"]:
                if row["remaining"]: typer.echo(f"- {row['scope']}: {row['remaining']} {'change needs' if row['remaining']==1 else 'changes need'} review.")
...
def remember(content: str, scope: Optional[str] = typer.Option(None,"--project"), canonical: Optional[str] = typer.Option(None, "--replace", help="Revise the uniquely matching user-owned memory by ID, title, or text."), from_: Optional[list[str]] = typer.Option(None, "--from", help="Attach exact archived message evidence; repeatable."), json_output: bool = typer.Option(False, "--json", help="Emit the structured mutation result.")):
    result = _safe(lambda: remember_data(sys.stdin.read() if content == "-" else content, scope, canonical, from_ or ()))
    _emit(result) if json_output else typer.echo(f"{'Memory unchanged' if result['status'] == 'unchanged' else result['status'].title()+' memory'} for `{result['scope']}`" + (f" with {result['evidence']} evidence link." if result["evidence"] == 1 else f" with {result['evidence']} evidence links." if result["evidence"] else "."))
...
def forget(memory: str = typer.Argument(...,metavar="MEMORY",help="Unique memory ID, title, or text."), scope: Optional[str] = typer.Option(None,"--project"), dry_run: bool = typer.Option(False, "--dry-run"), json_output: bool = typer.Option(False, "--json", help="Emit the structured purge result.")):
    result = _safe(lambda: forget_data(memory, scope, dry_run))
    _emit(result) if json_output else typer.echo(f"{'Would forget' if dry_run else 'Forgot'} `{result['id']}` from `{result['scope']}`; {'would purge' if dry_run else 'purged'} {result['revisions']} stored revision record{'s' if result['revisions'] != 1 else ''} and {result['evidence']} evidence link{'s' if result['evidence'] != 1 else ''}." + (" Run again without --dry-run to confirm." if dry_run else ""))
...
def backup(target: Optional[str] = typer.Argument(None), json_output: bool = typer.Option(False, "--json", help="Emit structured backup details.")):
    result = _safe(lambda: backup_data(target))
    _emit(result) if json_output else typer.echo(f"Memory backed up to `{result['path']}`: {result['canonicals']} {'memory' if result['canonicals']==1 else 'memories'} and {result['revisions']} revisions across {result['scopes']} {'project' if result['scopes']==1 else 'projects'} ({result['bytes']} bytes).")
...
def restore(source: str, yes: bool = typer.Option(False, "--yes", help="Restore after preserving the current memory."), json_output: bool = typer.Option(False, "--json", help="Emit structured backup details.")):
    result = _safe(lambda: restore_data(source, yes))
    _emit(result) if json_output else typer.echo((f"Would restore `{result['path']}`: {result['canonicals']} {'memory' if result['canonicals']==1 else 'memories'} and {result['revisions']} revisions across {result['scopes']} {'project' if result['scopes']==1 else 'projects'}. Run again with --yes to confirm." if not yes else f"Memory restored from `{result['path']}`; the previous memory was preserved at `{result['rescue']}`."))
...
def adopt_scope(scope: str, checkout: Optional[str] = typer.Option(None), yes: bool = typer.Option(False,"--yes"), json_output: bool = typer.Option(False,"--json")):
    result = _safe(lambda: adopt_scope_data(scope,checkout,yes))
    _emit(result) if json_output else typer.echo((f"Would bind `{result['checkout']}` to `{result['scope']}` ({result['canonicals']} canonicals, {result['sources']} sources). Run again with --yes to confirm." if not yes else f"Checkout `{result['checkout']}` now uses memory scope `{result['scope']}`."))
@memory.command()
def enable(scope: Optional[str] = typer.Option(None,"--project"), all_: bool = typer.Option(False, "--all", help="Check every discovered project instead of only the selected project.")):
    ...
    if all_ and scope is not None:
        typer.echo("--all cannot be combined with --project",err=True)
        raise typer.Exit(1)
    result = _safe(lambda: enable_data(scope, all_))
    checked = f"{len(result['scopes'])} discovered {'project' if len(result['scopes']) == 1 else 'projects'}" if all_ else f"project `{result['scope']}`"
...
@memory.command(hidden=True)
def plan(content: bool = typer.Option(False,"--content",help="Include current source content for agent resolution."),scope: Optional[str] = typer.Option(None)):
    """Create a deterministic synchronization plan."""
    _emit(_safe(lambda:(scan_store(s:=_scope(scope)),plan_data(content,scope=s))[1]))
@memory.command(hidden=True)
...
def current(scope: Optional[str] = typer.Option(None,"--project"), query: Optional[str] = typer.Option(None), owned: bool = typer.Option(False, "--owned", help="Return only memories explicitly saved with remember."), json_output: bool = typer.Option(False, "--json", help="Emit structured memory and origin records.")):
    scope = _safe(lambda: _scope(scope))
    rows = _safe(lambda: (sync_data(scope), [r for r in _current(scope, query) if not owned or any(o["provider"] == "user" for o in r["origins"])])[1])
...
def install_hook(remove: bool = typer.Option(False, "--remove"), status: bool = typer.Option(False, "--status")):
    try: configs = context_hook_config(remove, status)
        ...
        typer.echo(f"Invalid agent hook settings: {e}",err=True)
        raise typer.Exit(1)
...
def project(target: Optional[str] = typer.Option(None, "--target"), scope: Optional[str] = typer.Option(None), write: bool = typer.Option(False, "--write"), remove: bool = typer.Option(False, "--remove"), content: bool = typer.Option(False, "--content", help="Include rendered Markdown in a preview.")):
    if write and remove:
        typer.echo("--write and --remove are mutually exclusive",err=True)
        raise typer.Exit(1)
    scope = _safe(lambda: _scope(scope))
    result = _safe(lambda: remove_projection(target, scope) if remove else write_projection(target, scope) if write else projection_data(target, scope))
    _emit(result if content else {k:v for k,v in result.items() if k != "content"})
...
def history(memory: str = typer.Argument(...,metavar="MEMORY",help="Source ID or unique memory ID, title, or text."), scope: Optional[str] = typer.Option(None,"--project"), json_output: bool = typer.Option(False, "--json", help="Emit structured identity and revision records.")):
    db = _safe(connect)
    sources, canonicals = _safe(lambda: (db.execute("SELECT id,provider,scope,locator,hash,active,observed_at FROM sources WHERE substr(id,1,length(?))=? LIMIT 2", (memory,memory)).fetchall(), db.execute("SELECT id,scope,hash,updated_at FROM canonicals WHERE substr(id,1,length(?))=? LIMIT 2", (memory,memory)).fetchall()))
    if not sources and not canonicals: canonicals = [_safe(lambda:_select_canonical(db,_scope(scope),memory))]
    if len(sources)+len(canonicals)!=1:
        db.close()
        typer.echo("memory id is missing or ambiguous",err=True)
        raise typer.Exit(1)
    ...
    rows = _safe(lambda: [{**dict(r),**({"evidence":_evidence(db,record["id"],r["hash"])} if kind == "canonical" else {})} for r in db.execute(f"SELECT hash,content,observed_at FROM {table} WHERE {column}=? ORDER BY observed_at", (record["id"],))])
    db.close()
    result = dict(kind=kind, **record, revisions=rows)
    _emit(result) if json_output else typer.echo(_render_history(result))
@memory.callback()
def home(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        try: typer.echo(_render_home(_health_data()))
            ...
            typer.echo(f"Memory health is unavailable: {e}",err=True)
            raise typer.Exit(1)
