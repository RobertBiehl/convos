...
def _plain(value):
    ...
    return value
def _safe(value):
    clean,findings=inspect(value)
    return _plain(clean),len(findings)
...
def git_data(scope):
    ...
    status=(_git(scope,"status","--short") or "").splitlines()
    cleaned=[_safe(row) for row in status[:80]]
    subject,subject_redactions=_safe(_git(scope,"log","-1","--format=%s") or "")
    safe=[row for row,_ in cleaned]
    ...
    return dict(data,redactions=sum(n for _,n in cleaned)+subject_redactions+n)
...
def _turns(raw,quota,context):
    ...
    for mid,role,content,created_at in raw:
        if used>=quota: break
        clean,n=_safe(content)
        clipped=_clip(clean,min(context,quota-used))
        shown.append(dict(message_id=mid,role=role,created_at=created_at,content=clipped))
        used,redactions=used+len(clipped),redactions+n
    return shown,used,redactions
def packet_data(scope=".",days=None,limit=4,turns=6,context=1200,budget=16000):
    scope=scope_path(scope)
    drain_hooks()
    clause=" AND m.created_at>=CURRENT_TIMESTAMP-(?*INTERVAL '1 day')" if days else ""
    params=[str(scope),str(scope)+"/",*([days] if days else []),limit]
...
def replay_data(ref,around="",limit=20,context=2000,activity=100):
    ...
    with contextlib.closing(_db()) as db:
        ...
        if len(cs)!=1: raise ValueError("Conversation reference is missing or ambiguous")
        cid,title,source,cwd=cs[0]
    ...
    counts={kind:sum(e["kind"]==kind for _,e in events) for kind in ("tool","edit")}
...
def render(data):
    ...
    if git["repository"]:
        ...
        [typer.echo(f"  - `{row}`") for row in git["status"]]
        git["status_truncated"] and typer.echo("  - ... status truncated")
    if not data["sessions"]: return typer.echo("\n## Archive\n\nNo matching project conversations found.")
...
def replay_cmd(conversation:str,around:str=typer.Option("","--around","-a"),limit:int=typer.Option(20,"-n",min=1,max=100),context:int=typer.Option(2000,"-c",min=1,max=10000),activity:int=typer.Option(100,"--activity",min=0,max=200),fmt:str=typer.Option("text","-f","--format")):
    ...
    try: data=replay_data(conversation,around,limit,context,activity)
        ...
        typer.echo(str(e),err=True)
        raise typer.Exit(1)
    if fmt=="json": return typer.echo(json.dumps(data,default=str))
    if fmt!="text":
    ...
    for message in data["messages"]:
        ...
        for event in message["activity"]: lines.extend((f"  {'TOOL '+(event['name'] or '?')+' '+(event['status'] or '?') if event['kind']=='tool' else 'EDIT '+(event['type'] or '?')+' '+(event['path'] or '?')+' ['+event['evidence_status']+']'} [{event['id']}]",*(f"    {key}: {event[key]}" for key in (("input","output") if event["kind"]=="tool" else ("before","after")) if event[key] is not None)))
    count=lambda n,s:f"{n} {s}{'' if n==1 else 's'}"
    lines.append(f"\n{count(len(data['messages']),'message')}, {count(data['counts']['tools'],'tool')}, {count(data['counts']['edits'],'edit')}{', activity truncated' if data['activity_truncated'] else ''}")
    typer.echo("\n".join(lines))
def register(app): [app.command(name)(command) for name,command in (("resume",resume_cmd),("replay",replay_cmd))]
