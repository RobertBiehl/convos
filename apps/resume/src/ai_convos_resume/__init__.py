...
def replay_data(ref,around="",limit=20,context=2000,activity=100):
    ...
    if selected and activity:
        ...
        event_rows=db.execute(f"""WITH chosen(id,pos) AS (VALUES {values}), events AS (
            ...
            UNION ALL SELECT c.pos,'edit',e.id,e.message_id,e.created_at,e.file_path,e.edit_type,NULL,e.old_content,e.content,COALESCE(v.status,'legacy_unverified') evidence_status FROM file_edits e JOIN chosen c ON c.id=e.message_id LEFT JOIN provenance.file_edit_evidence v ON v.file_edit_id=e.id)
