...
def test_memory_distribution_metadata_and_public_help():
    ...
    assert project["readme"] == "README.md" and set(project["urls"]) == {"Documentation","Repository"} and f"convos>={project['version']},<0.12" in project["dependencies"] and project["entry-points"]["convos.init"] == {"memory":"ai_convos_memory:initialize"} and project["entry-points"]["convos.remote"] == {"memory":"ai_convos_memory:remote_bridge"} and memory_module.remote_bridge()["v"] == 3 and memory_module.remote_bridge()["objects"] == {"memory.canonical"} and set(memory_module.remote_bridge()) == {"v","schema","objects","records","accept"} and "memory" not in core["optional-dependencies"]
