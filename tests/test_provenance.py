...
def test_v4_indexes_native_sessions_classifies_recovery_and_removes_exact_startup_stubs(tmp_path):
    ...
    db.executemany("INSERT INTO conversations VALUES (?,?,'x',NULL,NULL,NULL,NULL,NULL,NULL,?)",[("keep","codex",meta("native")),("stub","codex",meta("stub")),("sub","codex",meta("child","subagent")),("history","claude-code",recovered),("foreign","codex",meta("native"))]); db.executemany("INSERT INTO messages VALUES (?,?,?, ?,NULL,NULL,NULL,?,NULL,NULL)",[("keep-m","keep","user","real prompt","{}"),("stub-m","stub","user","# AGENTS.md instructions for /repo","{}"),("sub-u","sub","user","<environment_context>","{}"),("sub-a","sub","assistant","work","{}"),("history-m","history","user","old prompt",recovered),("repair-m","keep","unknown","",repair)]); db.execute("INSERT INTO remote.row_origins VALUES ('conversations','foreign','w','author','device','foreign','event','conversations:foreign',NULL,NULL)"); rebuild_fts_index(db); before=archive_state(db)[1]; db.execute("DROP TABLE provider_sessions; UPDATE core_schema SET version=3"); db.close()
    db=duckdb.connect(str(path)); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and set(db.execute("SELECT source,session_id,conversation_id FROM provider_sessions").fetchall())=={("codex","native","keep"),("codex","child","sub")}; assert not db.execute("SELECT 1 FROM conversations WHERE id='stub'").fetchone() and not db.execute("SELECT 1 FROM messages WHERE id='stub-m'").fetchone() and db.execute("SELECT 1 FROM conversations WHERE id='sub'").fetchone(); assert json.loads(db.execute("SELECT CAST(metadata AS VARCHAR) FROM conversations WHERE id='history'").fetchone()[0])["capture_mode"]=="history" and json.loads(db.execute("SELECT CAST(metadata AS VARCHAR) FROM messages WHERE id='repair-m'").fetchone()[0])["capture_mode"]=="recovery"; assert archive_state(db)[1]>before and {tuple(r) for r in db.execute("SELECT kind,entity FROM archive_changes WHERE entity IN ('stub','stub-m','history','history-m','repair-m')").fetchall()}=={("conversations","stub"),("messages","stub-m"),("conversations","history"),("messages","history-m"),("messages","repair-m")}; db.close(); assert path.with_name("v3.db.pre-v4.bak").is_file()
...
def test_v4_interruption_rolls_back_and_retry_matches_clean_migration(tmp_path,monkeypatch):
    path=tmp_path/"retry.db"; db=graph(path); db.execute("DROP TABLE provider_sessions; INSERT INTO conversations VALUES ('stub','codex','x',NULL,NULL,NULL,NULL,NULL,NULL,'{\"session_id\":\"stub\",\"session_kind\":\"main\"}'); INSERT INTO messages VALUES ('m','stub','user','# AGENTS.md instructions for /repo',NULL,NULL,NULL,'{}',NULL,NULL); UPDATE core_schema SET version=3"); db.close(); real=core_module._archive_touch; monkeypatch.setattr(core_module,"_archive_touch",lambda *_:(_ for _ in ()).throw(RuntimeError("interrupted"))); db=duckdb.connect(str(path))
    with pytest.raises(RuntimeError,match="interrupted"): init_schema(db)
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==3 and db.execute("SELECT id FROM conversations").fetchone()[0]=="stub" and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name='provider_sessions'").fetchone(); monkeypatch.setattr(core_module,"_archive_touch",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and not db.execute("SELECT 1 FROM conversations").fetchone(); db.close()


def test_v2_unaffected_archive_skips_backup_and_rewrite(tmp_path):
    path=tmp_path/"ready.db"; db=graph(path); user="author"; row_id=remote_id(user,"conversations","c"); db.execute("INSERT INTO conversations VALUES (?,'codex','ready',NULL,NULL,NULL,NULL,NULL,NULL,'{}')",[row_id]); db.execute("INSERT INTO remote.row_origins VALUES ('conversations',?,'w',?,'d','c','e','conversations:c',NULL,NULL)",[row_id,user]); before=archive_state(db)[1]; db.execute("UPDATE core_schema SET version=1"); db.close(); db=duckdb.connect(str(path)); init_schema(db); assert db.execute("SELECT id FROM conversations").fetchone()[0]==row_id and archive_state(db)[1]==before and db.execute("SELECT version FROM core_schema").fetchone()[0]==4; db.close(); assert not path.with_name("ready.db.pre-v2.bak").exists()
...
def test_v2_resumes_durable_archive_change_cutover(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(migrations_module,"migrate_remote_changes",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and db.execute("SELECT entity FROM archive_changes WHERE kind='conversations'").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name='core_remote_id_map'").fetchone(); db.close()
...
def test_v2_resumes_durable_batched_data_cutover(tmp_path,monkeypatch):
    ...
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and db.execute("SELECT id FROM conversations").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name LIKE 'core_remote_%'").fetchone(); db.close()
...
def test_v2_migrates_workspace_scoped_rows_and_unblocks_provenance_replay(tmp_path,monkeypatch):
    ...
    db.execute("UPDATE core_schema SET version=1"); rebuild_fts_index(db); docid=db.execute("SELECT docid FROM fts_main_messages.docs WHERE name=?",[old("messages","m")]).fetchone()[0]; before=archive_state(db)[1]; db.close(); monkeypatch.setattr(core_module,"rebuild_fts_index",lambda _:(_ for _ in ()).throw(AssertionError("FTS rebuild was unnecessary"))); db=duckdb.connect(str(path)); init_schema(db); assert path.with_name("archive.db.pre-v2.bak").is_file() and db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and db.execute("SELECT id FROM conversations").fetchone()[0]==remote_id(user,"conversations","c") and db.execute("SELECT id,conversation_id FROM messages").fetchone()==(remote_id(user,"messages","m"),remote_id(user,"conversations","c")) and db.execute("SELECT id,message_id FROM file_edits").fetchone()==(remote_id(user,"file_edits","e"),remote_id(user,"messages","m")) and archive_state(db)[1]>before
...
def test_v2_resumes_required_fts_rebuild_after_data_commit(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(core_module,"rebuild_fts_index",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==4 and not db.execute("SELECT 1 FROM core_migrations").fetchone() and db.execute("SELECT fts_main_messages.match_bm25(?, 'findable')",[remote_id(user,"messages","m")]).fetchone()[0] is not None; db.close()
