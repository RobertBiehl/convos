...
def test_local_ingest_cannot_overwrite_a_signed_remote_projection(tmp_path):
    ...
    core_module.upsert(db,incoming)
    with pytest.raises(ValueError,match="signed attachment body conflicts"): core_module.index_attachment_body(db,"a",body,3)
    values=["t","m","shell","{}",'"done"',"complete",None,None]; origin={"workspace_id":"w","author_user_id":"u","author_device_id":"d","source_row_id":"source","source_event_id":"revision","content_key":"tool_calls:source","observed_at":None}
    with pytest.raises(ValueError,match="archive ownership conflict"): project_archive_row(db,"tool_calls",ARCHIVE_COLUMNS["tool_calls"],values)
    with pytest.raises(ValueError,match="archive ownership conflict"): project_archive_row(db,"tool_calls",ARCHIVE_COLUMNS["tool_calls"],["local",*values[1:]],origin|{"source_row_id":"local"})
    project_archive_row(db,"tool_calls",ARCHIVE_COLUMNS["tool_calls"],values,origin)
    assert db.execute("SELECT output,status FROM tool_calls WHERE id='t'").fetchone()==('"done"',"complete")
    assert db.execute("SELECT output,status FROM tool_calls WHERE id='fresh'").fetchone()==('"done"',"complete") and db.execute("SELECT content_hash,size FROM attachment_bodies").fetchone()==("old",3)
    db.close()

def test_local_ingest_prunes_remote_subtree_but_commits_unrelated_conversations(tmp_path,capsys):
    db=graph(tmp_path/"archive.db"); db.execute("INSERT INTO conversations(id,source,title,metadata) VALUES ('remote','codex','signed','{}'); INSERT INTO remote.row_origins(table_name,physical_row_id,workspace_id,author_user_id,author_device_id,source_row_id,source_event_id,content_key) VALUES ('conversations','remote','w','u','d','source','event','conversations:source')")
    conv=lambda id,title:dict(id=id,source="codex",title=title,created_at=None,updated_at=None,model=None,cwd=None,git_branch=None,project_id=None,metadata="{}")
    msg=lambda id,cid:dict(id=id,conversation_id=cid,role="user",content=id,thinking=None,created_at=None,model=None,metadata="{}",parent_id=None)
    tool=lambda id,mid:dict(id=id,message_id=mid,tool_name="shell",input="{}",output="{}",status="complete",duration_ms=None,created_at=None)
    core_module.upsert(db,core_module.ParseResult(convs=[conv("remote","overwrite"),conv("local","kept")],msgs=[msg("foreign-child","remote"),msg("local-message","local")],tools=[tool("foreign-tool","foreign-child"),tool("local-tool","local-message")]))
    assert db.execute("SELECT title FROM conversations WHERE id='remote'").fetchone()[0]=="signed" and db.execute("SELECT title FROM conversations WHERE id='local'").fetchone()[0]=="kept" and not db.execute("SELECT 1 FROM messages WHERE id='foreign-child'").fetchone() and db.execute("SELECT 1 FROM tool_calls WHERE id='local-tool'").fetchone() and "skipped 3 local row(s)" in capsys.readouterr().err; db.close()

def test_manual_archive_backup_is_checkpointed_validated_and_private(tmp_path,monkeypatch,capsys):
    path,body=tmp_path/"core.db",tmp_path/"body"; body.write_bytes(b"retained"); db=graph(path); db.execute("INSERT INTO conversations(id,source,metadata) VALUES ('kept','codex','{}'); INSERT INTO messages(id,conversation_id,role,metadata) VALUES ('m','kept','user','{}'); INSERT INTO attachments(id,message_id,path,size) VALUES ('a','m',?,8)",[str(body)]); core_module.index_attachment_body(db,"a",body,8); db.close(); monkeypatch.setattr(core_module,"DB_PATH",path); core_module.backup(); saved=next(tmp_path.glob("core.db.pre-manual-*.bak")); bundle=saved.with_name(saved.name+".attachments"); check=duckdb.connect(str(saved),read_only=True)
    assert check.execute("SELECT id FROM conversations").fetchone()[0]=="kept" and saved.stat().st_mode&0o777==0o600 and (bundle/digest(b"retained")).read_bytes()==b"retained" and json.loads((bundle/"manifest.json").read_text())["database_sha256"]==core_module._file_sha256(saved) and str(saved) in capsys.readouterr().out; check.close()

def test_manual_backup_refuses_missing_body_before_publishing_database(tmp_path,monkeypatch):
    path,body=tmp_path/"core.db",tmp_path/"body"; body.write_bytes(b"retained"); db=graph(path); db.execute("INSERT INTO conversations(id,source) VALUES ('c','test'); INSERT INTO messages(id,conversation_id,role) VALUES ('m','c','user'); INSERT INTO attachments(id,message_id,path,size) VALUES ('a','m',?,8)",[str(body)]); core_module.index_attachment_body(db,"a",body,8); db.close(); body.unlink(); monkeypatch.setattr(core_module,"DB_PATH",path)
    with pytest.raises(ValueError,match="attachment body unavailable"): core_module.backup()
    assert not list(tmp_path.glob("core.db.pre-manual-*.bak"))

def test_manual_backup_does_not_silently_migrate_archive(tmp_path,monkeypatch):
...
def test_legacy_attachment_migration_backs_up_external_bodies(tmp_path):
...
def test_repair_donor_restores_missing_rows_without_overwriting_new_local_content(tmp_path):
    import shutil
    path,backup=tmp_path/"live.db",tmp_path/"donor.bak"
    db=graph(path); db.execute("INSERT INTO conversations(id,source,title,metadata) VALUES ('c','codex','old','{}'); INSERT INTO messages(id,conversation_id,role,content,metadata) VALUES ('m','c','user','original','{}')"); db.close(); shutil.copyfile(path,backup)
    db=duckdb.connect(str(path)); db.execute("UPDATE conversations SET title='new local' WHERE id='c'; DELETE FROM messages; INSERT INTO messages(id,conversation_id,role,content,metadata) VALUES ('new','c','user','concurrent local','{}')"); db.close()
    for _ in range(2): core_module.merge_archive_backup(path,backup,page=1)
    with duckdb.connect(str(path),read_only=True) as db:
        assert db.execute("SELECT id,title FROM conversations").fetchall()==[("c","new local")]
        assert db.execute("SELECT id,content FROM messages ORDER BY id").fetchall()==[("m","original"),("new","concurrent local")]
    wrong=tmp_path/"wrong.db"; graph(wrong).close()
    with pytest.raises(ValueError,match="different archive"): core_module.merge_archive_backup(path,wrong)
...
def test_v4_identity_conflict_is_retained_and_bound_deterministically_with_backup(tmp_path):
    ...
    init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT COUNT(*) FROM conversations").fetchone()[0]==2 and db.execute("SELECT source,session_id,conversation_id FROM provider_sessions").fetchall()==[("codex","same","a")]; db.close(); assert path.with_name("conflict.db.pre-v4.bak").is_file()
...
def test_v3_migration_quarantines_duplicate_legacy_edit_scopes(tmp_path):
    path=tmp_path/"duplicates.db"; db=graph(path); db.execute("INSERT INTO conversations VALUES ('c','codex','x',NULL,NULL,NULL,NULL,NULL,NULL,'{}'); INSERT INTO messages VALUES ('m','c','assistant','x',NULL,NULL,NULL,'{}',NULL,NULL); INSERT INTO file_edits VALUES ('e','m','x.py','write','x',NULL,NULL); INSERT INTO provenance.files VALUES ('f1',NULL,'one','external'),('f2',NULL,'two','external'); INSERT INTO provenance.local_facts VALUES ('edit.observed','e'); DROP TABLE provenance.file_edit_files; CREATE TABLE provenance.file_edit_files(file_edit_id VARCHAR,file_id VARCHAR,old_content_hash VARCHAR,new_content_hash VARCHAR,evidence VARCHAR,PRIMARY KEY(file_edit_id,file_id)); INSERT INTO provenance.file_edit_files VALUES ('e','f1','a','b','captured_exact'),('e','f2','a','b','captured_exact'); UPDATE core_schema SET version=2"); db.close(); db=duckdb.connect(str(path)); init_schema(db); row=db.execute("SELECT x.file_id,x.evidence,f.path,f.kind,f.repository FROM provenance.file_edit_files x JOIN provenance.files f ON f.id=x.file_id WHERE x.file_edit_id='e'").fetchone(); expected=digest({"repository":None,"path":row[2]}); assert row==(expected,"legacy_scope_conflict",f"external/{digest('e')[:24]}/unknown","external",None) and db.execute("SELECT 1 FROM provenance.local_facts WHERE kind='file.observed' AND entity=?",[expected]).fetchone() and db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION; db.close(); assert path.with_name("duplicates.db.pre-v3.bak").is_file()

def test_v3_refuses_foreign_signed_duplicate_before_mutating_archive(tmp_path):
    path=tmp_path/"foreign-duplicates.db"; db=graph(path); root,device=identity("root"),identity("device"); user=public_id(root["sign_public"]); cert=certificate(root,user,device); fact={"v":1,"kind":"edit.observed","id":"e","state":"active","data":{"turn":"m","file":"f1","repository":None,"old_content_hash":"old","new_content_hash":"new","evidence":"captured_exact"}}; proof=row_proof(device,user,"w",1,fact); proof_id=project_row_proof(db,proof,root["sign_public"],cert)
    db.execute("INSERT INTO conversations(id,source,metadata) VALUES ('c','codex','{}'); INSERT INTO messages(id,conversation_id,role,metadata) VALUES ('m','c','assistant','{}'); INSERT INTO file_edits VALUES ('e','m','x.py','write','x',NULL,NULL); INSERT INTO provenance.files VALUES ('f1',NULL,'one','external'),('f2',NULL,'two','external'); INSERT INTO provenance.local_facts VALUES ('edit.observed','e')"); db.execute("INSERT INTO remote.provenance_origins VALUES ('edit.observed','e','w',?,'e',?)",(user,proof_id)); db.execute("DROP TABLE provenance.file_edit_files; CREATE TABLE provenance.file_edit_files(file_edit_id VARCHAR,file_id VARCHAR,old_content_hash VARCHAR,new_content_hash VARCHAR,evidence VARCHAR,PRIMARY KEY(file_edit_id,file_id)); INSERT INTO provenance.file_edit_files VALUES ('e','f1','old','new','captured_exact'),('e','f2','old','new','captured_exact'); UPDATE core_schema SET version=2"); before=(db.execute("SELECT * FROM provenance.file_edit_files ORDER BY file_id").fetchall(),db.execute("SELECT * FROM remote.row_proofs WHERE id=?",(proof_id,)).fetchone(),db.execute("SELECT * FROM remote.provenance_origins").fetchall(),db.execute("SELECT * FROM provenance.local_facts ORDER BY ALL").fetchall()); db.close(); db=duckdb.connect(str(path))
    with pytest.raises(ValueError,match="refused to rewrite ambiguous foreign signed edit"): init_schema(db)
    after=(db.execute("SELECT * FROM provenance.file_edit_files ORDER BY file_id").fetchall(),db.execute("SELECT * FROM remote.row_proofs WHERE id=?",(proof_id,)).fetchone(),db.execute("SELECT * FROM remote.provenance_origins").fetchall(),db.execute("SELECT * FROM provenance.local_facts ORDER BY ALL").fetchall()); assert after==before and db.execute("SELECT version FROM core_schema").fetchone()[0]==2; verify_row_proof(proof,fact,cert,root["sign_public"]); db.close(); backup=path.with_name("foreign-duplicates.db.pre-v3.bak"); check=duckdb.connect(str(backup),read_only=True); assert check.execute("SELECT * FROM provenance.file_edit_files ORDER BY file_id").fetchall()==before[0] and check.execute("SELECT version FROM core_schema").fetchone()[0]==2; check.close()

def test_legacy_conflict_healer_never_rewrites_foreign_signed_fact(tmp_path):
    path=tmp_path/"foreign-healer.db"; db=graph(path); db.execute("INSERT INTO conversations(id,source,metadata) VALUES ('c','codex','{}'); INSERT INTO messages(id,conversation_id,role,metadata) VALUES ('m','c','assistant','{}'); INSERT INTO file_edits VALUES ('e','m','x.py','write','x',NULL,NULL); INSERT INTO provenance.files VALUES ('foreign-file',NULL,'external/x.py','external'); INSERT INTO provenance.file_edit_files VALUES ('e','foreign-file','old','new','legacy_scope_conflict'); INSERT INTO provenance.local_facts VALUES ('edit.observed','e'),('file.observed','foreign-file'); INSERT INTO remote.provenance_origins VALUES ('edit.observed','e','w','author','e','proof')"); before=(db.execute("SELECT * FROM provenance.file_edit_files").fetchall(),db.execute("SELECT * FROM provenance.files").fetchall(),db.execute("SELECT * FROM provenance.local_facts ORDER BY ALL").fetchall(),archive_state(db)); init_schema(db); assert before==(db.execute("SELECT * FROM provenance.file_edit_files").fetchall(),db.execute("SELECT * FROM provenance.files").fetchall(),db.execute("SELECT * FROM provenance.local_facts ORDER BY ALL").fetchall(),archive_state(db)); db.close()

def test_v8_migrates_embedding_profile_and_normalized_semantic_ancestry(tmp_path):
    path=tmp_path/"v7.db"; db=graph(path); ancestor="a"*64; proof=json.dumps({"ancestors":[ancestor]}); db.execute("INSERT INTO conversations(id,source,metadata) VALUES ('c','codex','{}'); INSERT INTO messages(id,conversation_id,role,metadata,embedding) VALUES ('m','c','user','{}',?)",([0.0]*768,)); db.execute("INSERT INTO remote.provider_session_aliases VALUES ('w','author','object','revision','codex','session','[\"c\"]','c',?)",(proof,)); db.execute("DROP TABLE remote.semantic_ancestors; DROP TABLE embedding_state; ALTER TABLE messages ALTER embedding TYPE FLOAT[768]; UPDATE core_schema SET version=7"); db.close(); db=duckdb.connect(str(path)); init_schema(db); profile=json.loads(db.execute("SELECT CAST(profile AS VARCHAR) FROM embedding_state").fetchone()[0]); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT column_type FROM (DESCRIBE messages) WHERE column_name='embedding'").fetchone()[0]=="FLOAT[]" and profile==core_module._EPROFILES["llama"] and db.execute("SELECT object_kind,child_revision,ancestor_revision FROM remote.semantic_ancestors").fetchone()==("provider.session","revision",ancestor); db.close(); backup=duckdb.connect(str(path.with_name("v7.db.pre-v8.bak")),read_only=True); assert backup.execute("SELECT version FROM core_schema").fetchone()[0]==7 and backup.execute("SELECT column_type FROM (DESCRIBE messages) WHERE column_name='embedding'").fetchone()[0]=="FLOAT[768]"; backup.close()

def test_v9_adds_authoritative_stale_retrieval_state_with_backup(tmp_path):
    path=tmp_path/"v8.db"; db=graph(path); db.execute("INSERT INTO conversations(id,source) VALUES ('c','test'); INSERT INTO messages(id,conversation_id,role,content) VALUES ('m','c','user','needle'); DROP TABLE retrieval_state; UPDATE core_schema SET version=8"); db.close(); db=duckdb.connect(str(path)); init_schema(db)
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT messages_generation,fts_generation,fts_definition_hash FROM retrieval_state").fetchone()[1:]==(None,core_module._FTS_DEF); db.close()
    backup=path.with_name("v8.db.pre-v9.bak"); assert backup.is_file() and backup.with_name(backup.name+".attachments").is_dir() and duckdb.connect(str(backup),read_only=True).execute("SELECT version FROM core_schema").fetchone()[0]==8
...
def test_v2_resumes_durable_archive_change_cutover(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(migrations_module,"migrate_remote_changes",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT entity FROM archive_changes WHERE kind='conversations'").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name='core_remote_id_map'").fetchone(); db.close()
...
def test_v2_resumes_durable_batched_data_cutover(tmp_path,monkeypatch):
    ...
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT id FROM conversations").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name LIKE 'core_remote_%'").fetchone(); db.close()
...
def test_v2_migrates_workspace_scoped_rows_and_unblocks_provenance_replay(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match="dependency missing"): project(db,event,mapper)
    db.execute("UPDATE core_schema SET version=1"); rebuild_fts_index(db); docid=db.execute("SELECT docid FROM fts_main_messages.docs WHERE name=?",[old("messages","m")]).fetchone()[0]; before=archive_state(db)[1]; db.close(); monkeypatch.setattr(core_module,"rebuild_fts_index",lambda _:(_ for _ in ()).throw(AssertionError("FTS rebuild was unnecessary"))); db=duckdb.connect(str(path)); init_schema(db); assert path.with_name("archive.db.pre-v2.bak").is_file() and db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and db.execute("SELECT id FROM conversations").fetchone()[0]==remote_id(user,"conversations","c") and db.execute("SELECT id,conversation_id FROM messages").fetchone()==(remote_id(user,"messages","m"),remote_id(user,"conversations","c")) and db.execute("SELECT id,message_id FROM file_edits").fetchone()==(remote_id(user,"file_edits","e"),remote_id(user,"messages","m")) and archive_state(db)[1]>before
...
def test_v2_resumes_required_fts_rebuild_after_data_commit(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(core_module,"rebuild_fts_index",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==core_module.CORE_VERSION and not db.execute("SELECT 1 FROM core_migrations").fetchone() and db.execute("SELECT fts_main_messages.match_bm25(?, 'findable')",[remote_id(user,"messages","m")]).fetchone()[0] is not None; db.close()
...
def test_remote_change_gets_canonical_identity_and_move_keeps_it(tmp_path):
    root=repo(tmp_path/"source"); git(root,"remote","add","origin","https://example.com/acme/project.git"); path=tmp_path/"core.db"; db=core(path,root,[(root/"x.py","write","one\n",None)]); db.close(); capture(path); db=duckdb.connect(str(path)); stable=repository(root,db)["id"]; git(root,"remote","set-url","origin","git@example.com:other/fork.git"); changed=repository(root,db)["id"]; assert changed!=stable
    moved=tmp_path/"moved"; __import__("shutil").move(root,moved); assert repository(moved,db)["id"]==changed
    fork=tmp_path/"fork"; subprocess.run(("git","clone","-q",str(moved),str(fork)),check=True); git(fork,"remote","set-url","origin","git@example.com:other/fork.git"); assert repository(fork,db)["id"]==changed
...
def test_rule_migration_merges_proxy_ids_and_keeps_every_link(tmp_path,monkeypatch):
    ...
    db=duckdb.connect(str(path)); assert len(db.execute("SELECT id FROM provenance.repositories").fetchall())==2==query(db,"conversation_changes","c")[0]["repositories"]; files=query(db,"changeset_files","m"); db.execute("DELETE FROM core_migrations WHERE name='git_identity'"); db.close()
    monkeypatch.undo(); session("token-b"); core_module.canonicalize_repositories(path); db=duckdb.connect(str(path)); git(main,"worktree","add","-q",str(tmp_path/"fresh"))
...
def test_migration_keeps_loopback_rows_without_matching_local_evidence(tmp_path):
    ...
    assert core_module.rekey_repositories(db)==0 and db.execute("SELECT id FROM provenance.repositories").fetchall()==[(old,)]; db.close()

def test_rekey_moves_only_local_observations_to_canonical_destination(tmp_path):
    ...
    assert db.execute("SELECT 1 FROM provenance.local_facts WHERE kind='repository.observed' AND entity=?",[new]).fetchone()
    assert db.execute("SELECT 1 FROM provenance.local_facts WHERE kind='file.observed' AND entity=?",[new_file]).fetchone()
    assert not db.execute("SELECT 1 FROM information_schema.tables WHERE table_schema='provenance' AND table_name='rekeyed'").fetchone()
...
def test_repository_capture_reconciles_a_bound_proxy_id_before_writing(tmp_path):
    ...
    db.execute("INSERT INTO provenance.repositories VALUES (?,?,?, ?,NULL,NULL)",[old,current["lineage"],json.dumps(current["roots"]),json.dumps([loop])]); db.execute("INSERT INTO provenance.repository_checkouts VALUES (?,?,?,?,?)",[current["checkout"],old,str(root),current["branch"],current["head"]]); db.execute("INSERT INTO provenance.local_facts VALUES ('repository.observed',?)",[old]); db.close()
...
def test_proxy_fallback_uses_configured_url_without_applying_a_losing_rewrite(tmp_path,proxy):
    ...
    git(root,"config","url.https://wrong.example/.insteadOf","https://example.com/")
    git(root,"config",f"url.{proxy}.insteadOf","https://example.com/acme/")
    assert repository(root)["remotes"]==["https://example.com/acme/project"]

def test_canonical_remote_overrides_stale_checkout_binding(tmp_path):
    root=repo(tmp_path/"checkout")
    git(root,"remote","add","origin","https://example.com/acme/project.git")
    current=repository(root)
    known={"roots":{},"checkouts":{current["checkout"]:"old"},"checkout_roots":{},"lineages":{"old":current["lineage"]},"aliases":{current["id"]:"old"}}
    assert repository(root,known)["id"]==current["id"]
    git(root,"remote","remove","origin")
    assert repository(root,known)["id"]=="old"

def test_config_migration_is_backed_up_atomic_and_skips_unchanged_rules(tmp_path,monkeypatch):
    root=repo(tmp_path/"checkout")
    git(root,"remote","add","origin","https://old.example/team/project.git")
    path=tmp_path/"archive.db"
    core(path,root,[(root/"x.py","write","one\n",None)]).close()
    capture(path)
    with duckdb.connect(str(path),read_only=True) as db:
        old=db.execute("SELECT id FROM provenance.repositories").fetchone()[0]
        fingerprint=db.execute("SELECT state FROM core_migrations WHERE name='git_identity'").fetchone()[0]
    config=tmp_path/"settings"; config.mkdir()
    (config/"config.json").write_text(json.dumps({"git_identity":{"host_aliases":{"old.example":"new.example"}}}))
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(config))
    backup=core_module._migration_backup
    def interrupted(*args): raise OSError("backup interrupted")
    monkeypatch.setattr(core_module,"_migration_backup",interrupted)
    with pytest.raises(OSError,match="backup interrupted"): core_module.canonicalize_repositories(path)
    with duckdb.connect(str(path),read_only=True) as db:
        assert db.execute("SELECT id FROM provenance.repositories").fetchall()==[(old,)]
        assert db.execute("SELECT state FROM core_migrations WHERE name='git_identity'").fetchone()==(fingerprint,)
    monkeypatch.setattr(core_module,"_migration_backup",backup)
    assert core_module.canonicalize_repositories(path)>0
    saved=next(tmp_path.glob("archive.db.pre-git-identity-*.bak"))
    with duckdb.connect(str(saved),read_only=True) as db: assert db.execute("SELECT id FROM provenance.repositories").fetchall()==[(old,)]
    (config/"config.json").write_text(json.dumps({"git_identity":{"host_aliases":{"old.example":"final.example","new.example":"final.example"}}}))
    assert core_module.canonicalize_repositories(path)>0
    with duckdb.connect(str(path),read_only=True) as db:
        assert db.execute("SELECT remotes FROM provenance.repositories").fetchone()==('["https://final.example/team/project"]',)
        assert db.execute("SELECT version FROM core_schema").fetchone()==(17,)
    def unexpected(*a,**k): pytest.fail("unchanged rules performed Git inspection or mutation")
    monkeypatch.setattr(core_module,"_git_run",unexpected)
    monkeypatch.setattr(core_module,"_migration_backup",unexpected)
    monkeypatch.setattr(core_module,"rekey_repositories",unexpected)
    assert core_module.canonicalize_repositories(path)==0
...
def test_custom_ssh_port_requires_explicit_scm_mapping(tmp_path):
    ...
    assert ssh==["https://code.example.com:7999/ACME/project"] and repository(root)["remotes"]==["https://code.example.com/scm/ACME/project"]
