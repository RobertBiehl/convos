...
def test_v2_migrates_workspace_scoped_rows_and_unblocks_provenance_replay(tmp_path):
    ...
    db.execute("UPDATE core_schema SET version=1"); rebuild_fts_index(db); before=archive_state(db)[1]; db.close(); db=duckdb.connect(str(path)); init_schema(db); assert path.with_name("archive.db.pre-v2.bak").is_file() and db.execute("SELECT version FROM core_schema").fetchone()[0]==2 and db.execute("SELECT id FROM conversations").fetchone()[0]==remote_id(user,"conversations","c") and db.execute("SELECT id,conversation_id FROM messages").fetchone()==(remote_id(user,"messages","m"),remote_id(user,"conversations","c")) and db.execute("SELECT id,message_id FROM file_edits").fetchone()==(remote_id(user,"file_edits","e"),remote_id(user,"messages","m")) and archive_state(db)[1]>before
    new_edit=remote_id(user,"file_edits","e"); new_link=digest({"checkpoint":"cp","edit":new_edit}); assert project(db,event,mapper) and db.execute("SELECT file_edit_id FROM provenance.file_edit_files").fetchone()[0]==new_edit and db.execute("SELECT file_edit_id FROM provenance.checkpoint_edits").fetchone()[0]==new_edit and {r[0] for r in db.execute("SELECT physical_entity FROM remote.provenance_origins").fetchall()}=={new_edit,new_link} and db.execute("SELECT COUNT(*) FROM remote.row_origins").fetchone()[0]==3 and db.execute("SELECT fts_main_messages.match_bm25(?, 'findable')",[remote_id(user,"messages","m")]).fetchone()[0] is not None; db.close()
