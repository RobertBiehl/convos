...
def test_v4_identity_conflict_is_retained_and_bound_deterministically_with_backup(tmp_path):
    ...
    init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and db.execute("SELECT COUNT(*) FROM conversations").fetchone()[0]==2 and db.execute("SELECT source,session_id,conversation_id FROM provider_sessions").fetchall()==[("codex","same","a")]; db.close(); assert path.with_name("conflict.db.pre-v4.bak").is_file()
...
def test_v8_migrates_embedding_profile_and_normalized_semantic_ancestry(tmp_path):
    path=tmp_path/"v7.db"; db=graph(path); ancestor="a"*64; proof=json.dumps({"ancestors":[ancestor]}); db.execute("INSERT INTO conversations(id,source,metadata) VALUES ('c','codex','{}'); INSERT INTO messages(id,conversation_id,role,metadata,embedding) VALUES ('m','c','user','{}',?)",([0.0]*768,)); db.execute("INSERT INTO remote.provider_session_aliases VALUES ('w','author','object','revision','codex','session','[\"c\"]','c',?)",(proof,)); db.execute("DROP TABLE remote.semantic_ancestors; DROP TABLE embedding_state; ALTER TABLE messages ALTER embedding TYPE FLOAT[768]; UPDATE core_schema SET version=7"); db.close(); db=duckdb.connect(str(path)); init_schema(db); profile=json.loads(db.execute("SELECT CAST(profile AS VARCHAR) FROM embedding_state").fetchone()[0]); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and db.execute("SELECT column_type FROM (DESCRIBE messages) WHERE column_name='embedding'").fetchone()[0]=="FLOAT[]" and profile==core_module._EPROFILES["llama"] and db.execute("SELECT object_kind,child_revision,ancestor_revision FROM remote.semantic_ancestors").fetchone()==("provider.session","revision",ancestor); db.close(); backup=duckdb.connect(str(path.with_name("v7.db.pre-v8.bak")),read_only=True); assert backup.execute("SELECT version FROM core_schema").fetchone()[0]==7 and backup.execute("SELECT column_type FROM (DESCRIBE messages) WHERE column_name='embedding'").fetchone()[0]=="FLOAT[768]"; backup.close()
...
def test_v2_resumes_durable_archive_change_cutover(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(migrations_module,"migrate_remote_changes",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and db.execute("SELECT entity FROM archive_changes WHERE kind='conversations'").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name='core_remote_id_map'").fetchone(); db.close()
...
def test_v2_resumes_durable_batched_data_cutover(tmp_path,monkeypatch):
    ...
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and db.execute("SELECT id FROM conversations").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name LIKE 'core_remote_%'").fetchone(); db.close()
...
def test_v2_migrates_workspace_scoped_rows_and_unblocks_provenance_replay(tmp_path,monkeypatch):
    ...
    db.execute("UPDATE core_schema SET version=1"); rebuild_fts_index(db); docid=db.execute("SELECT docid FROM fts_main_messages.docs WHERE name=?",[old("messages","m")]).fetchone()[0]; before=archive_state(db)[1]; db.close(); monkeypatch.setattr(core_module,"rebuild_fts_index",lambda _:(_ for _ in ()).throw(AssertionError("FTS rebuild was unnecessary"))); db=duckdb.connect(str(path)); init_schema(db); assert path.with_name("archive.db.pre-v2.bak").is_file() and db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and db.execute("SELECT id FROM conversations").fetchone()[0]==remote_id(user,"conversations","c") and db.execute("SELECT id,conversation_id FROM messages").fetchone()==(remote_id(user,"messages","m"),remote_id(user,"conversations","c")) and db.execute("SELECT id,message_id FROM file_edits").fetchone()==(remote_id(user,"file_edits","e"),remote_id(user,"messages","m")) and archive_state(db)[1]>before
...
def test_v2_resumes_required_fts_rebuild_after_data_commit(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(core_module,"rebuild_fts_index",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==8 and not db.execute("SELECT 1 FROM core_migrations").fetchone() and db.execute("SELECT fts_main_messages.match_bm25(?, 'findable')",[remote_id(user,"messages","m")]).fetchone()[0] is not None; db.close()
