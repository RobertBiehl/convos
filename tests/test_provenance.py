...
def test_v2_resumes_durable_archive_change_cutover(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(migrations_module,"migrate_remote_changes",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==5 and db.execute("SELECT entity FROM archive_changes WHERE kind='conversations'").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name='core_remote_id_map'").fetchone(); db.close()
...
def test_v2_resumes_durable_batched_data_cutover(tmp_path,monkeypatch):
    ...
    assert db.execute("SELECT version FROM core_schema").fetchone()[0]==5 and db.execute("SELECT id FROM conversations").fetchone()[0]==new and not db.execute("SELECT 1 FROM information_schema.tables WHERE table_name LIKE 'core_remote_%'").fetchone(); db.close()
...
def test_v2_migrates_workspace_scoped_rows_and_unblocks_provenance_replay(tmp_path,monkeypatch):
    ...
    db.execute("UPDATE core_schema SET version=1"); rebuild_fts_index(db); docid=db.execute("SELECT docid FROM fts_main_messages.docs WHERE name=?",[old("messages","m")]).fetchone()[0]; before=archive_state(db)[1]; db.close(); monkeypatch.setattr(core_module,"rebuild_fts_index",lambda _:(_ for _ in ()).throw(AssertionError("FTS rebuild was unnecessary"))); db=duckdb.connect(str(path)); init_schema(db); assert path.with_name("archive.db.pre-v2.bak").is_file() and db.execute("SELECT version FROM core_schema").fetchone()[0]==5 and db.execute("SELECT id FROM conversations").fetchone()[0]==remote_id(user,"conversations","c") and db.execute("SELECT id,conversation_id FROM messages").fetchone()==(remote_id(user,"messages","m"),remote_id(user,"conversations","c")) and db.execute("SELECT id,message_id FROM file_edits").fetchone()==(remote_id(user,"file_edits","e"),remote_id(user,"messages","m")) and archive_state(db)[1]>before
...
def test_v2_resumes_required_fts_rebuild_after_data_commit(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(core_module,"rebuild_fts_index",real); init_schema(db); assert db.execute("SELECT version FROM core_schema").fetchone()[0]==5 and not db.execute("SELECT 1 FROM core_migrations").fetchone() and db.execute("SELECT fts_main_messages.match_bm25(?, 'findable')",[remote_id(user,"messages","m")]).fetchone()[0] is not None; db.close()
