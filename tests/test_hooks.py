...
def test_sync_refetches_tool_ended_chatgpt_at_same_timestamp(hooks,monkeypatch):
    _,data=hooks; data.mkdir(); monkeypatch.setattr(cli,"STATE_PATH",data/"sync_state.json"); cid=cli.gen_id("chatgpt","active"); when=cli.ts_any(100); conv=dict(id=cid,source="chatgpt",title="T",created_at=when,updated_at=when,model=None,cwd=None,git_branch=None,project_id=None,metadata=json.dumps({"remote_update_time":100})); msg=dict(id="tool",conversation_id=cid,role="tool",content="running",thinking=None,created_at=when,model=None,metadata="{}",parent_id=None); db=duckdb.connect(str(data/"convos.db")); cli.init_schema(db); cli.upsert(db,cli.ParseResult([conv],[msg])); db.close(); captured=[]
