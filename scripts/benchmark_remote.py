...
def run(source,keep=False):
    root=Path(tempfile.mkdtemp(prefix="convos-remote-benchmark-")); a,b=root/"author",root/"holder"; core,result,ok=a/"data/convos.db",{"protocol":1,"source_bytes":source.stat().st_size},False
    try:
        phase("copy-on-write archive clone"); clone(source,core); started=time.perf_counter(); db=duckdb.connect(str(core)); init_schema(db); archive_counts={table:db.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] for table in ("conversations","messages","tool_calls","attachments","artifacts","file_edits")}; db.close(); backup,(migrated,_)=core.with_name(core.name+".pre-v1.bak"),duck_settle(core); result|={"archive_counts":archive_counts,"migration_seconds":elapsed(started),"migration_backup_bytes":backup.stat().st_size if backup.exists() else 0,"migration_growth_bytes":migrated-result["source_bytes"]}
        relay=relay_connect(relay_path:=root/"relay.db"); direct=transport(relay); remote.request,remote.drain_hooks=direct,lambda:None; phase("create two-device personal workspace"); alice,recovery=remote.setup_client("http://local","benchmark","author",root=a); remote.setup_client("http://local","benchmark","holder",recovery,root=b); origin=remote.workspace(alice,"Personal")
        ...
        interrupted_attempt_seconds,remote.request=elapsed(started),direct; phase("idempotent interruption recovery"); started=time.perf_counter(); remote.sync_once(a,True); interruption_recovery_seconds=elapsed(started); initial_size,initial_counts,initial_replicas=(*duck_settle(core),relay.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]); phase("isolated relay replica page walk"); relay_page_replicas,page_seconds=relay_page_walk(relay,alice,origin); result|={"interrupted_attempt_seconds":interrupted_attempt_seconds,"interruption_recovery_seconds":interruption_recovery_seconds,"initial_signing_seconds":sum(t["seconds"] for t in timings if t["phase"]=="sign"),"core_replication_growth_bytes":initial_size-result["source_bytes"],"core_counts":initial_counts,"relay_page_replicas":relay_page_replicas,"relay_page_seconds":round(page_seconds,3),"relay_page_rows_per_second":round(relay_page_replicas/page_seconds,3)}
        phase("one-row incremental sync"); db=duckdb.connect(str(core)); row=list(db.execute("SELECT * FROM conversations ORDER BY id LIMIT 1").fetchone()); row[4]=(row[4] or row[3])+timedelta(microseconds=1); db.execute("BEGIN"); project_archive_row(db,"conversations",ARCHIVE_COLUMNS["conversations"],row); db.execute("COMMIT"); db.close(); started=time.perf_counter(); remote.sync_once(a); result|={"incremental_seconds":elapsed(started),"incremental_replicas":relay.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]-initial_replicas}
        phase("fresh-holder verification and projection"); verification_replicas=relay.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]; started=time.perf_counter(); remote.sync_once(b,True); verification_seconds=elapsed(started); holder_size,_=duck_settle(b/"data/convos.db"); result|={"verification_seconds":verification_seconds,"verification_replicas":verification_replicas,"verification_rows_per_second":round(verification_replicas/verification_seconds,3),"holder_core_bytes":holder_size}
        phase("empty-relay re-home and full holder repair"); fresh=relay_connect(fresh_path:=root/"fresh-relay.db"); remote.request,started=transport(fresh),time.perf_counter(); bob,_=remote.rehome_client(remote.load(b),"http://fresh",b); replacement=remote.workspace(bob,"Personal"); state=state_connect(b/"remote/state.db"); remote.bind_origin(bob,state,replacement,origin,b); state.close(); remote.sync_once(b,True); result|={"empty_relay_rebuild_seconds":elapsed(started),"empty_relay_replicas":fresh.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]}
        state_bytes=sqlite_settle(state:=state_connect(a/"remote/state.db"),a/"remote/state.db"); state.close(); relay_wire_bytes=sum(relay.execute(f"SELECT COALESCE(SUM(length({column})),0) FROM {table}").fetchone()[0] for table,column in (("events","envelope"),("row_replicas","envelope"),("blob_replicas","ciphertext"),("origin_bundles","envelope"))); relay_bytes=sqlite_settle(relay,relay_path); relay.close(); fresh_relay_bytes=sqlite_settle(fresh,fresh_path); fresh.close(); result|={"state_bytes":state_bytes,"relay_wire_bytes":relay_wire_bytes,"relay_bytes":relay_bytes,"fresh_relay_bytes":fresh_relay_bytes,"timings":{phase:{"calls":len(rows),"seconds":round(sum(r["seconds"] for r in rows),3),"records":sum(r["records"] for r in rows)} for phase in {t["phase"] for t in timings} for rows in [[t for t in timings if t["phase"]==phase]]},"proof_overhead_ratio":round(result["core_replication_growth_bytes"]/result["source_bytes"],6)}
