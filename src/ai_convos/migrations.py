...
def _relation(db,name,id_column,kind):
    target,keys,parts=f"core_remote_{name.replace('.','_')}",','.join('x.'+r[0] for r in db.execute(f"DESCRIBE {name}").fetchall() if r[0]!=id_column and r[3]=='PRI'),_table_parts(db,name)
    return (db.execute(f"CREATE OR REPLACE TABLE {target} AS SELECT *,0::UINTEGER _batch FROM {name} WHERE FALSE; CREATE OR REPLACE TEMP TABLE _convos_relation_origins AS SELECT physical_row_id,new_id FROM _convos_origins WHERE table_name='{kind}'; CREATE OR REPLACE TEMP TABLE _convos_relation_rows AS SELECT old_id,winner FROM core_remote_id_rows WHERE kind='{kind}'"),[(db.execute(f"INSERT INTO {target} SELECT x.* REPLACE (COALESCE(o.new_id,x.{id_column}) AS {id_column}),{batch} FROM {name} x LEFT JOIN _convos_relation_origins o ON o.physical_row_id=x.{id_column} WHERE hash(COALESCE(o.new_id,x.{id_column}){','+keys if keys else ''})%{parts}={batch} QUALIFY row_number() OVER (PARTITION BY COALESCE(o.new_id,x.{id_column}){','+keys if keys else ''} ORDER BY COALESCE((SELECT winner FROM _convos_relation_rows w WHERE w.old_id=x.{id_column}),FALSE) DESC)=1"),db.execute("COMMIT; BEGIN")) for batch in range(parts)],target)[-1]
...
def migrate_remote_ids(db,archive_columns):
    ...
    for table,batch in ((table,batch) for table in archive_columns for batch in _batches(db,table)):
    ...
    changed,has_fts,direct,rebuild=(changed:={r[0] for r in db.execute("SELECT DISTINCT table_name FROM _convos_origins WHERE physical_row_id<>new_id").fetchall()}),(has_fts:=bool(db.execute("SELECT 1 FROM information_schema.schemata WHERE schema_name='fts_main_messages'").fetchone())),(direct:=has_fts and "messages" in changed and not fts_needs_rebuild(db)),has_fts and "messages" in changed and not direct
...
def migrate_remote_changes(db):
    ...
    for batch in range(parts:=_table_parts(db,"core_remote_id_map")): db.execute(f"""INSERT INTO archive_changes SELECT m.kind,m.new_id,max(c.generation) FROM core_remote_id_map m JOIN archive_changes c ON (c.kind,c.entity)=(m.kind,m.old_id) WHERE {(scope:=f'm.old_id<>m.new_id AND hash(m.kind,m.old_id)%{parts}={batch}')} GROUP BY m.kind,m.new_id ON CONFLICT(kind,entity) DO UPDATE SET generation=greatest(archive_changes.generation,excluded.generation); DELETE FROM archive_changes c USING core_remote_id_map m WHERE (c.kind,c.entity)=(m.kind,m.old_id) AND {scope}; INSERT OR REPLACE INTO archive_changes SELECT DISTINCT m.kind,m.new_id,{generation} FROM core_remote_id_map m WHERE {scope}{'; COMMIT; BEGIN' if batch+1<parts else ''}""")
