...
def _git_remotes(root): return sorted({remote for line in _git_maybe(root,"config","--get-regexp",r"^remote\..*\.(push)?url$").decode(errors="replace").splitlines() if " " in line and not (url:=line.split(" ",1)[1]).startswith(("/","file:")) and (remote:=_remote(url))})  # configured URLs: `remote -v` applies url.*.insteadOf, which can hold a local proxy and its credential
...
def project_logical_row(db,row,proof,proof_id,native=False,touch=True,parent_map=None,defer=False,preserve=True,local=None,record_origin=True):
    ...
    if table in PROVENANCE_KINDS:
        ...
        if defer and provenance_issue(db,value,mapped,replace_file): return defer(proof_id)
...
def _archive_string_check(db,schema,table,column): return (typer.echo(f"string {schema}.{table}.{column}",err=True),db.execute(f'SELECT count(*),sum(coalesce(length(CAST("{column.replace(chr(34),chr(34)*2)}" AS VARCHAR)),0)) FROM "{schema.replace(chr(34),chr(34)*2)}"."{table.replace(chr(34),chr(34)*2)}"').fetchone(),1)[-1]
def _verify_worker(path): return (lambda db,indexes,strings:(typer.echo(json.dumps(dict(tables=len(indexes),keys=sum(_archive_index_check(db,*row) for row in indexes),strings=sum(_archive_string_check(db,*row) for row in strings)))),db.close())[0])(db:=open_db(path=path,read_only=True,wait=300,purpose="archive.verify"),db.execute("SELECT schema_name,table_name,constraint_column_names[1] FROM duckdb_constraints() WHERE constraint_type='PRIMARY KEY' AND len(constraint_column_names)=1 AND schema_name IN ('main','provenance','remote') ORDER BY 1,2").fetchall(),db.execute("SELECT table_schema,table_name,column_name FROM information_schema.columns WHERE table_schema IN ('main','provenance','remote') AND data_type IN ('VARCHAR','JSON') ORDER BY 1,2,ordinal_position").fetchall())
def verify_archive(path): return (required((result:=subprocess.run([sys.executable,"-m","ai_convos","verify-worker",str(Path(path).resolve())],capture_output=True,text=True)).returncode==0,ValueError(f"Archive verification failed before write: {(result.stderr.strip().splitlines() or [f'process exit {result.returncode}'])[-1]}")),json.loads(result.stdout))[-1]
def verify(): typer.echo(json.dumps(verify_archive(DB_PATH),sort_keys=True))
...
def reset_archive_sync(path,user,device):
    ...
    with contextlib.closing((verify_archive(path),get_db(path=path,purpose='sync.cutover'))[-1]) as db:
...
def reset_archive_sync_db(db,user,device):
    ...
    db.execute("CREATE OR REPLACE TEMP TABLE sync_local AS SELECT 'conversations' kind,c.id FROM conversations c JOIN provider_sessions s ON s.conversation_id=c.id UNION SELECT 'messages',m.id FROM messages m JOIN provider_sessions s ON s.conversation_id=m.conversation_id UNION SELECT 'tool_calls',t.id FROM tool_calls t JOIN messages m ON m.id=t.message_id JOIN provider_sessions s ON s.conversation_id=m.conversation_id UNION SELECT 'attachments',a.id FROM attachments a JOIN messages m ON m.id=a.message_id JOIN provider_sessions s ON s.conversation_id=m.conversation_id UNION SELECT 'artifacts',a.id FROM artifacts a JOIN provider_sessions s ON s.conversation_id=a.conversation_id UNION SELECT 'file_edits',e.id FROM file_edits e JOIN messages m ON m.id=e.message_id JOIN provider_sessions s ON s.conversation_id=m.conversation_id")
...
def enqueue_hook(source, payload):
    path,root=(Path(payload["transcript_path"]).expanduser().resolve() if payload.get("transcript_path") else next((p.resolve() for p in sorted(hook_root(source).expanduser().resolve().rglob(f"{payload.get('session_id')}/session.jsonl")) if p.is_file()),None) if source=="muse" and re.fullmatch(r"[0-9a-f-]{1,64}",payload.get("session_id") or "") else None),hook_root(source).expanduser().resolve()
...
for _fn,_name,_hidden in ((capture,"hook",True),(capture,"capture",True),(drain_hooks_cmd,"drain-hooks",True),(_verify_worker,"verify-worker",True),(init,None,False),(fts,None,False),(search,None,False),(read_cmd,"read",False),(query_cmd,"query",False),(embed_cmd,"embed",False),(doctor,None,False),(verify,None,False),(install_skills,None,False),(install_hooks,"install-hooks",False),(export,None,False),(backup,None,False),(sync,None,False),(sql,None,False)): app.command(_name,hidden=_hidden)(_fn)
