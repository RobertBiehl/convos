...
def retired_projection(kind,physical,body,extra): return [physical if k=='id' else extra[k] if k in extra else body['data'][k] for k in ARCHIVE_COLUMNS[kind]]
...
def retire_parser_rows(db):
    for kind,label in (('tool_calls','tool'),('messages','message'),('conversations','conversation')):
        ...
        for at in range(0,len(plans),500):
            ...
            _insert_pages(db,'core_parser_retire',[p[:4]+(provenance_digest(body),) for p,claim in zip(page,claims) if (body:=bodies[claim]) is not None and (p[4] is None or provenance_digest(body)==p[4])],mode=' OR IGNORE')
        blocked=' OR '.join([*(f"EXISTS(SELECT 1 FROM {table} c WHERE c.{column}=p.physical"+(" AND NOT EXISTS(SELECT 1 FROM core_parser_retire v WHERE v.physical=c.id)" if table==kind else "")+")" for table,refs in ARCHIVE_FKS.items() for column,parent in refs if parent==kind),*(["EXISTS(SELECT 1 FROM provenance.file_edit_evidence e WHERE e.tool_call_id=p.physical)"] if kind=='tool_calls' else []),"FALSE"])
        ...
        for at in range(0,len(plans),500):
            ...
            selected=[(p,body) for p,claim in zip(page,claims) if p[0] in raws and (body:=bodies[claim]) is not None and provenance_digest(body)==p[4]]
            if selected:
...
def project_logical_rows(db,items,defer=False):
    ...
    (record_parser_replicas(db,evidence),retire_parser_rows(db))
...
def session_bindings(conn): return {(source,session):cid for source,session,cid in conn.execute("SELECT source,session_id,conversation_id FROM provider_sessions").fetchall()}|{(source,session,'legacy'):ids for source,session,ids in conn.execute("SELECT c.source,p.session_id,list(c.id ORDER BY c.id) FROM conversations c JOIN provider_sessions p ON p.source=c.source AND p.session_id=json_extract_string(c.metadata,'$.session_id') WHERE c.id<>p.conversation_id AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE o.table_name='conversations' AND o.physical_row_id=c.id) GROUP BY c.source,p.session_id").fetchall()}
...
def prepare_message_lineage(db,r):
    ...
    for cid in {current[mid]['conversation_id'] for mid,*_ in r.message_lineage if mid in current}:
        ...
        aliases,(candidates)=(aliases:=[v[0] for v in db.execute("SELECT id FROM conversations c WHERE source='claude-code' AND id<>? AND (json_extract_string(metadata,'$.session_id') IN (SELECT UNNEST(?)) OR id IN (SELECT UNNEST(?))) AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE o.table_name='conversations' AND o.physical_row_id=c.id)",[cid,list({session}|{v[5] for v in r.message_lineage}),list({v[4] for v in r.message_lineage})]).fetchall()] if conv['source']=='claude-code' and session else []),([(current[mid],{**current[mid],'id':gen_id('claude-code',f'{old}:{index}'),'conversation_id':old,'parent_id':parent,'model':model,'metadata':metadata,'created_at':stamp}) for mid,index,parent_index,filtered,old_path,old_session,raw_time in r.message_lineage if mid in current and current[mid]['conversation_id']==cid for old in aliases for parent in ({gen_id('claude-code',f'{old}:{parent_index}'),None} if filtered and parent_index is not None else {gen_id('claude-code',f'{old}:{parent_index}') if parent_index is not None else None}) for metadata in ('{}',json.dumps({'provider_index':json.loads(current[mid]['metadata'])['provider_index']})) for model in {current[mid]['model'],'claude' if current[mid]['role']=='assistant' else None} for stamp in {current[mid]['created_at'],datetime.fromisoformat(raw_time.replace('Z','+00:00')) if raw_time else None}])
...
def parse_claude_code_session(jsonl: Path, bindings=None) -> dict:
    ...
    turns,(indices)=(turns:=[(idx,i,e,(c:=extract_content(e["message"].get("content",e["message"].get("text","")))),ts_from_iso(e.get("timestamp")),gen_id(src,f"{cid}:{idx}")) for idx,(i,e) in enumerate(msg_events)]),({mid:(idx,bool(c['text'] or c['tools'])) for idx,i,e,c,ts,mid in turns})
