...
def prepare_edit_lineage(db,r):
    ...
    ids={e['id'] for pair in r.edit_lineage for e in pair}
    paths,routes=captured_edit_paths(db,ids),dict(db.execute("SELECT file_edit_id,route FROM provenance.file_edit_scopes WHERE file_edit_id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps(list(ids))]).fetchall())
    variants=lambda row,anchor:[{**row,'file_path':path} for ref in [row['id'] if row['id'] in paths else anchor['id']] for path in {row['file_path'],*([paths[ref]] if ref in paths and routes.get(ref) and os.path.normpath(row['file_path'])==routes[ref] and (row['id'] not in routes or routes[row['id']]==routes[ref]) else [])}]
    pairs=[(old,new) for original,current in r.edit_lineage for prior in variants(original,current) for snapshot in (prior,dict(zip(prior,_history_row('file_edits',list(prior.values()),tuple(v for i,v in enumerate(prior.values()) if i not in (0,5)))))) for parent in {snapshot['message_id'],current['message_id']} for old in variants({**snapshot,'message_id':parent},current) for new in variants(current,current)]
