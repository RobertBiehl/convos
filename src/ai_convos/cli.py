...
def retire_parser_rows(db):
    if tools:=[r[0] for r in db.execute("SELECT DISTINCT r.physical FROM (SELECT old_id physical,old_source AS source,author FROM parser_tool_lineage WHERE old_id IS NOT NULL UNION SELECT physical,source,author FROM parser_retired_rows WHERE kind='tool_calls') r JOIN remote.file_edit_evidence_claims s ON s.source_tool_call_id=r.source AND (s.author_user_id=r.author OR r.author='')").fetchall()]: _apply_signed_edit_evidence(db,tools=tools)
