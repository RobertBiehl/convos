...
def _core(path=None,read_only=False,ready=False,wait=30,*,purpose):
    with contextlib.closing(get_db(read_only,wait,path,purpose=purpose)) as db:
        (((ready) and (init_schema(db))),(yield db))
@contextlib.contextmanager
...
CREATE OR REPLACE VIEW current_file_edits AS WITH history AS MATERIALIZED (SELECT old_id FROM parser_edit_history) SELECT e.* FROM file_edits e WHERE NOT EXISTS(SELECT 1 FROM history h WHERE h.old_id=e.id);
...
def capture_repository(path,db_path=None):
    ...
    with _core(db_path,purpose="provenance.repository") as db,_transaction(db):
        ((_observe_checkout(db,repo)),(project_native_provenance(db,[record])))
    return repo
...
def project_archive_rows(db,table,columns,rows,preserve=True):
    ...
    ((((((((restore_parser_parents(db,[(table,v[0]) for v,o in rows])),((table=='conversations') and (record_tool_lineage(db,[(v[0],o['author_user_id'] if o else '',not o) for v,o in rows],"messages"))))),((table in ('conversations','messages')) and (reconcile_tool_lineage(db,ids if table=='conversations' else (),[o['source_row_id'] if o else v[0] for v,o in rows] if table=='messages' else (),"messages"))))),((table=='messages') and (record_tool_lineage(db,[(v[0],o['author_user_id'] if o else '',not o) for v,o in rows]))))),((table in ('messages','tool_calls')) and (reconcile_tool_lineage(db,ids if table=='messages' else (),[o['source_row_id'] if o else v[0] for v,o in rows] if table=='tool_calls' else ()))))
    if table=='messages': record_tool_lineage(db,[(v[0],o['author_user_id'] if o else '',not o) for v,o in rows],'file_edits')
    if table in ('messages','file_edits'): reconcile_tool_lineage(db,ids if table=='messages' else (),[o['source_row_id'] if o else v[0] for v,o in rows] if table=='file_edits' else (),'file_edits')
...
def project_provider_alias(db,record):
    ...
    return (_insert_pages(db,"remote.provider_session_aliases",[tuple(record[f] for f in fields)],fields,mode=" OR IGNORE"),_project_semantic_ancestors(db,"provider.session",record))[-1]
...
def record_tool_lineage(db,messages,kind="tool_calls",metadata=None):
    ...
    metadata,records=(metadata:=metadata if metadata is not None else {mid:json.loads(raw or '{}') for mid,raw in db.execute(f"SELECT id,metadata FROM {owner} WHERE id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps([mid for mid,author,native in messages])]).fetchall()}),[(mid,author,native,r['old_id'],r['old_hash'],r['current_id'],r['current_hash'],None,None) for mid,author,native in messages for meta in [metadata.get(mid)] if isinstance(meta,dict) and isinstance(meta.get(f'convos_{label}_lineage'),dict) and type(meta[f'convos_{label}_lineage'].get('v')) is int and meta[f'convos_{label}_lineage']['v']==1 and isinstance(meta[f'convos_{label}_lineage'].get('records'),list) for r in meta[f'convos_{label}_lineage']['records'] if isinstance(r,dict) and set(r)=={'old_id','old_hash','current_id','current_hash'} and all(isinstance(r[k],str) and re.fullmatch('[0-9a-f]{'+str(64 if k.endswith('hash') else 16)+'}',r[k]) for k in r) and r['old_id']!=r['current_id']]
    return (db.execute(f"DELETE FROM parser_{label}_lineage WHERE message_id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps([mid for mid,author,native in messages])]),_insert_pages(db,f"parser_{label}_lineage",records,mode=" OR IGNORE"))[-1]
def reconcile_tool_lineage(db,messages=(),tools=(),kind="tool_calls"):
    label={"tool_calls":"tool","file_edits":"edit","messages":"message"}[kind]
    if not messages and not tools: return
    rows=db.execute(f"SELECT * FROM parser_{label}_lineage WHERE message_id IN (SELECT json_extract_string(value,'$') FROM json_each(?)) OR old_source IN (SELECT json_extract_string(value,'$') FROM json_each(?)) OR new_source IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps(list(messages)),json.dumps(list(tools)),json.dumps(list(tools))]).fetchall()
    for author,native in {(r[1],r[2]) for r in rows}:
        for at in range(0,len(selected:=[r for r in rows if (r[1],r[2])==(author,native)]),500):
            page,sources,found=(page:=selected[at:at+500]),(sources:={r[i] for r in page for i in (3,5)}),db.execute(f"SELECT t.id,coalesce(o.source_row_id,t.id) FROM (SELECT id FROM {kind} UNION SELECT physical FROM parser_retired_rows WHERE kind='{kind}') t LEFT JOIN remote.row_origins o ON o.table_name='{kind}' AND o.physical_row_id=t.id WHERE o.author_user_id=? AND o.source_row_id IN (SELECT json_extract_string(value,'$') FROM json_each(?)) OR (? OR EXISTS(SELECT 1 FROM remote.local_row_bases b WHERE b.kind='{kind}' AND b.entity=t.id AND b.author=?)) AND o.physical_row_id IS NULL AND t.id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[author,json.dumps(list(sources)),native,author,json.dumps(list(sources))]).fetchall()
            bindings,claims,bodies=(bindings:={source:members[0][0] for source,group in itertools.groupby(sorted(found,key=lambda r:r[1]),key=lambda r:r[1]) if len(members:=list(group))==1}),(claims:=[(kind,physical,source,author,'active') for source,physical in bindings.items()]),{claim[2]:value for claim,value in typed_logical_rows(db,claims).items()}
            columns=('old_id','new_id','message_id','author','old_source','old_hash','new_source','new_hash')
            if changes:=[dict(zip(columns,(*resolved,r[0],author,*r[3:7]))) for r in page if (resolved:=(bindings[r[3]],bindings[r[5]]) if r[3] in bodies and r[5] in bodies and matching_logical_row(bodies[r[3]],r[4]) is not None and matching_logical_row(bodies[r[5]],r[6]) is not None else (None,None))!=tuple(r[7:])]: db.execute(f"UPDATE parser_{label}_lineage l SET old_id=x.old_id,new_id=x.new_id FROM UNNEST(from_json(?,?)) t(x) WHERE "+' AND '.join(f'l.{key}=x.{key}' for key in columns[2:]),[json.dumps(changes,separators=(',',':')),json.dumps([{key:'VARCHAR' for key in columns}])])
def rebuild_parser_lineage(db,kinds=(('tool_calls','messages','tool'),('file_edits','messages','edit'),('messages','conversations','message')),retire=True):
    for kind,owner,label in kinds:
        if not db.execute("SELECT 1 FROM information_schema.columns WHERE table_name=? AND column_name='metadata'",[owner]).fetchone(): continue
        rows=db.execute(f"SELECT c.id,coalesce(o.author_user_id,b.author,''),o.physical_row_id IS NULL FROM {owner} c LEFT JOIN remote.row_origins o ON o.table_name=? AND o.physical_row_id=c.id LEFT JOIN (SELECT entity,min(author) author FROM remote.local_row_bases WHERE kind=? GROUP BY entity HAVING count(*)=1) b ON b.entity=c.id WHERE json_exists(c.metadata,'$.convos_{label}_lineage')",[owner,owner]).fetchall()
        for at in range(0,len(rows),500): (record_tool_lineage(db,rows[at:at+500],kind),reconcile_tool_lineage(db,[r[0] for r in rows[at:at+500]],kind=kind))
    if retire: (restore_parser_parents(db),retire_parser_rows(db))
def record_parser_replicas(db,items):
    for kind,owner in (('tool_calls','messages'),('file_edits','messages'),('messages','conversations')):
        sources,((origins, natives))=(sources:=[r['id'] for r,p,pid,native,*maps in items if r['kind']==owner]),(dict(((a,s),v) for a,s,v in db.execute("SELECT author_user_id,source_row_id,physical_row_id FROM remote.row_origins WHERE table_name=? AND source_row_id IN (SELECT UNNEST(?))",[owner,sources]).fetchall()),{v[0] for v in db.execute(f"SELECT id FROM {owner} c WHERE id IN (SELECT UNNEST(?)) AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE o.table_name=? AND o.physical_row_id=c.id)",[sources,owner]).fetchall()})
        records=[(r['id'] if native and r['id'] in natives else origins.get((p['author_user_id'],r['id']),r['id'] if native else remote_id(p['author_user_id'],owner,r['id'])),p['author_user_id'],native) for r,p,pid,native,*maps in items if r['kind']==owner]
        (record_tool_lineage(db,records,kind),reconcile_tool_lineage(db,[v[0] for v in records],[r['id'] for r,p,pid,native,*maps in items if r['kind']==kind],kind))
...
def restore_parser_parents(db,targets=None):
    targets=targets if targets is not None else [(kind,None) for kind in ARCHIVE_FKS]
    while targets:
        restored=[]
        for table in {k for k,v in targets}:
            for column,parent in ARCHIVE_FKS.get(table,()):
                rows=db.execute(f"SELECT DISTINCT r.physical,r.projection,r.body,r.content_hash,r.source,r.author FROM {table} c JOIN (SELECT * FROM parser_retired_rows QUALIFY count(*) OVER(PARTITION BY kind,physical)=1) r ON r.kind=? AND r.physical=c.{column} LEFT JOIN {parent} p ON p.id=r.physical WHERE (c.id IN (SELECT UNNEST(?)) OR ?) AND p.id IS NULL",[parent,[v for k,v in targets if k==table],(table,None) in targets]).fetchall()
                if rows: (_insert_pages(db,parent,[retired_projection(parent,physical,json.loads(body),json.loads(raw)) for physical,raw,body,*_ in rows],ARCHIVE_COLUMNS[parent]),(claims:=[(parent,physical,source,author,'active') for physical,raw,body,expected,source,author in rows]),(found:=typed_logical_rows(db,claims)),required(all(provenance_digest(json.loads(body))==expected and matching_logical_row(found[claim],expected)==json.loads(body) for (physical,raw,body,expected,source,author),claim in zip(rows,claims)),ValueError('Retired parent body mismatch')),restored.extend((parent,physical) for physical,*_ in rows))
        targets=restored
        if restored: _archive_touch(db,restored)
def retire_parser_rows(db):
    if tools:=[r[0] for r in db.execute("SELECT DISTINCT r.physical FROM (SELECT old_id physical,old_source AS source,author FROM parser_tool_lineage WHERE old_id IS NOT NULL UNION SELECT physical,source,author FROM parser_retired_rows WHERE kind='tool_calls') r JOIN remote.file_edit_evidence_claims s ON s.source_tool_call_id=r.source AND (s.author_user_id=r.author OR r.author='') WHERE (NOT EXISTS(SELECT 1 FROM tool_calls t WHERE t.id=r.physical) OR NOT EXISTS(SELECT 1 FROM provenance.file_edit_evidence e WHERE e.tool_call_id=r.physical)) AND NOT EXISTS(SELECT 1 FROM remote.file_edit_evidence_ancestors a WHERE (a.workspace_id,a.author_user_id,a.object_id,a.ancestor_revision)=(s.workspace_id,s.author_user_id,s.object_id,s.revision))").fetchall()]: _apply_signed_edit_evidence(db,tools=tools)
    for kind,label in (('tool_calls','tool'),('messages','message'),('conversations','conversation')):
        fresh=(f"SELECT l.old_id,l.new_id,l.old_source,l.author,l.old_hash FROM parser_{label}_lineage l JOIN {kind} old ON old.id=l.old_id JOIN {kind} new ON new.id=l.new_id WHERE l.old_id<>l.new_id"+''.join(f" AND (new.{column} IS NULL OR EXISTS(SELECT 1 FROM {parent} p WHERE p.id=new.{column}))" for column,parent in ARCHIVE_FKS.get(kind,()))+" QUALIFY count(DISTINCT l.new_id) OVER(PARTITION BY l.old_id)=1") if kind!='conversations' else "SELECT c.id,min(n.conversation_id),coalesce(o.source_row_id,c.id),coalesce(o.author_user_id,''),NULL FROM parser_retired_rows h JOIN conversations c ON c.id=json_extract_string(h.projection,'$.conversation_id') JOIN messages n ON n.id=h.replacement LEFT JOIN remote.row_origins o ON o.table_name='conversations' AND o.physical_row_id=c.id WHERE h.kind='messages' AND c.id<>n.conversation_id AND (o.author_user_id=h.author OR o.author_user_id IS NULL AND (h.author='' OR EXISTS(SELECT 1 FROM remote.local_row_bases b WHERE b.kind='conversations' AND b.entity=c.id AND b.author=h.author))) AND NOT EXISTS(SELECT 1 FROM parser_retired_rows r WHERE r.kind='conversations' AND r.physical=c.id) GROUP BY c.id,o.source_row_id,o.author_user_id HAVING count(DISTINCT n.conversation_id)=1"
        plans=(db.execute("CREATE OR REPLACE TEMP TABLE core_parser_retire(physical VARCHAR PRIMARY KEY,replacement VARCHAR,source VARCHAR,author VARCHAR,expected VARCHAR)"),db.execute(f"{fresh} UNION SELECT r.physical,r.replacement,r.source,r.author,r.content_hash FROM parser_retired_rows r JOIN {kind} old ON old.id=r.physical JOIN {kind} replacement ON replacement.id=r.replacement WHERE r.kind='{kind}'"+''.join(f" AND (replacement.{column} IS NULL OR EXISTS(SELECT 1 FROM {parent} p WHERE p.id=replacement.{column}))" for column,parent in ARCHIVE_FKS.get(kind,()))+(f" AND NOT EXISTS(SELECT 1 FROM parser_{label}_lineage l WHERE l.new_id=r.physical AND l.old_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM parser_{label}_lineage outgoing WHERE outgoing.old_id=r.physical))" if kind!='conversations' else "")).fetchall())[1]
        for at in range(0,len(plans),500):
            page,claims,bodies=(page:=plans[at:at+500]),(claims:=[(kind,p[0],p[2],p[3],'active') for p in page]),typed_logical_rows(db,claims)
            _insert_pages(db,'core_parser_retire',[p[:4]+(provenance_digest(body),) for p,claim in zip(page,claims) if (body:=matching_logical_row(bodies[claim],p[4]) if p[4] is not None else bodies[claim]) is not None],mode=' OR IGNORE')
        blocked=' OR '.join([*(f"EXISTS(SELECT 1 FROM {table} c WHERE c.{column}=p.physical"+(" AND NOT EXISTS(SELECT 1 FROM core_parser_retire v WHERE v.physical=c.id)" if table==kind else "")+")" for table,refs in ARCHIVE_FKS.items() for column,parent in refs if parent==kind),*(["EXISTS(SELECT 1 FROM provenance.file_edit_evidence e WHERE e.tool_call_id=p.physical)"] if kind=='tool_calls' else []),"EXISTS(SELECT 1 FROM core_parser_retire r WHERE r.physical=p.replacement)","FALSE"])
        plans=db.execute(f"WITH RECURSIVE blocked(physical) AS (SELECT p.physical FROM core_parser_retire p WHERE {blocked}"+(" UNION SELECT m.parent_id FROM messages m JOIN blocked b ON b.physical=m.id WHERE m.parent_id IS NOT NULL" if kind=='messages' else "")+") SELECT p.* FROM core_parser_retire p WHERE NOT EXISTS(SELECT 1 FROM blocked b WHERE b.physical=p.physical)").fetchall()
        for at in range(0,len(plans),500):
            page,claims,bodies,raws=(page:=plans[at:at+500]),(claims:=[(kind,p[0],p[2],p[3],'active') for p in page]),typed_logical_rows(db,claims),{r[0]:r for r in db.execute(f"SELECT {','.join(ARCHIVE_COLUMNS[kind])} FROM {kind} WHERE id IN (SELECT UNNEST(?))",[[p[0] for p in page]]).fetchall()}
            selected=[(p,body) for p,claim in zip(page,claims) if p[0] in raws and (body:=matching_logical_row(bodies[claim],p[4])) is not None]
            if selected:
                with preserve_fact_heads(db,[(kind,p[0]) for p,body in selected]): (_insert_pages(db,'parser_retired_rows',[(kind,p[0],p[2],p[3],p[4],body,{k:v for k,v in zip(ARCHIVE_COLUMNS[kind],raws[p[0]]) if k not in ROW_FIELDS_V1[kind] or k in dict(ARCHIVE_FKS.get(kind,()))},p[1]) for p,body in selected],mode=' OR IGNORE'),db.execute(f"DELETE FROM {kind} WHERE id IN (SELECT UNNEST(?))",[[p[0] for p,body in selected]]),kind=='conversations' and db.executemany("UPDATE provider_sessions SET conversation_id=? WHERE conversation_id=?",[(p[1],p[0]) for p,body in selected]),_archive_touch(db,[(kind,p[0]) for p,body in selected]),restore_retired_proofs(db,[p[0] for p,body in selected]))
def repair_parent_links(db,local_user=None,parents=None):
    ...
    return changed
def _protect_native_replicas(db,items,defer,advance_native=None):
        ...
        if {"remote.row_proofs","remote.row_origins","remote.provenance_origins"}<=available:
            with contextlib.closing(donor.cursor()) as reader:
                ...
                while proofs:=reader.fetchmany(page):
                    ...
                    archive_yield(path)
    with _core(path,purpose="remote.repair.parser-lineage") as db,_transaction(db): rebuild_parser_lineage(db)
    return restored
...
def session_bindings(conn): return {(source,session):cid for source,session,cid in conn.execute("SELECT source,session_id,conversation_id FROM provider_sessions").fetchall()}|{(source,session,'legacy'):ids for source,session,ids in conn.execute("SELECT source,session_id,list(DISTINCT member ORDER BY member) FROM (SELECT c.source,p.session_id,c.id member FROM conversations c JOIN provider_sessions p ON p.source=c.source AND p.session_id=json_extract_string(c.metadata,'$.session_id') WHERE c.id<>p.conversation_id AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE o.table_name='conversations' AND o.physical_row_id=c.id) UNION SELECT a.source,p.session_id,json_extract_string(j.value,'$') FROM remote.provider_session_aliases a JOIN provider_sessions p ON p.source=a.source AND p.conversation_id=a.canonical_source_row_id JOIN remote.local_row_bases b ON b.kind='conversations' AND b.entity=p.conversation_id AND b.author=a.author_user_id,json_each(a.members) j) GROUP BY source,session_id").fetchall()}|{(source,cid,'message',index):mid for source,cid,index,mid in conn.execute("SELECT DISTINCT c.source,c.id,TRY_CAST(json_extract_string(m.metadata,'$.provider_index') AS BIGINT),m.id FROM conversations c JOIN provider_sessions p ON p.conversation_id=c.id JOIN messages m ON m.conversation_id=c.id WHERE TRY_CAST(json_extract_string(m.metadata,'$.provider_index') AS BIGINT)>=0 AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE (o.table_name='messages' AND o.physical_row_id=m.id) OR (o.table_name='conversations' AND o.physical_row_id=c.id)) QUALIFY count(DISTINCT m.id) OVER(PARTITION BY c.id,json_extract_string(m.metadata,'$.provider_index'))=1").fetchall()}|{(source,cid,'title'):title for source,cid,title in conn.execute("SELECT DISTINCT c.source,c.id,c.title FROM conversations c JOIN provider_sessions p ON p.conversation_id=c.id WHERE c.title IS NOT NULL AND NOT EXISTS(SELECT 1 FROM remote.row_origins o WHERE o.table_name='conversations' AND o.physical_row_id=c.id)").fetchall()}
...
def chatgpt_headers(cookies, base, ua, debug_profile: str | None = None):
    ...
    with contextlib.suppress(Exception):
        ((headers.update({**({"Authorization":f"Bearer {token}"} if (token:=(session:=fetch_json(f"{base}/api/auth/session", cookies, headers, timeout=10, retries=0, rate_limit_backoff=300)).get("accessToken")) else {}),**({"ChatGPT-Account-ID":aid} if (aid:=session.get("account",{}).get("id")) else {})})),((debug_profile) and (typer.echo(f"  chatgpt chrome profile={debug_profile} user={session.get('user', {}).get('email')}", flush=True))))
    return headers
...
class ParseResult:
    ...
    tool_lineage: list = field(default_factory=list)
    edit_lineage: list = field(default_factory=list)
    message_lineage: list = field(default_factory=list)
    ...
    def __iadd__(self,other): return ([getattr(self,name).extend(getattr(other,name)) for name in ("convs","msgs","tools","attachs","artifacts","edits","edit_evidence","tool_lineage","edit_lineage","message_lineage","failed_inputs")],self.input_sessions.update(other.input_sessions),self)[-1]
    def __add__(self,other): return ParseResult(**{name:[*getattr(self,name),*getattr(other,name)] for name in ("convs","msgs","tools","attachs","artifacts","edits","edit_evidence","tool_lineage","edit_lineage","message_lineage","failed_inputs")},input_sessions=self.input_sessions|other.input_sessions)
...
def parser_tool_lineage(pairs,kind="tool_calls",predictive=False): return sorted({(new['message_id'],old['id'],provenance_digest(parser_logical_row(kind,{k:json.dumps(v) if k in ROW_JSON_V1 else stamp if k=='created_at' else v for k,v in old.items()})),new['id'],provenance_digest(parser_logical_row(kind,new)))+((True,) if predictive else ()) for old,new in pairs if old['id']!=new['id'] for stamp in {old['created_at'],old['created_at'].astimezone().replace(tzinfo=None) if old['created_at'] is not None and old['created_at'].tzinfo else old['created_at']}})
def _edit_alias_pairs(source,cid,members,rows): return [({**edit,'id':gen_id(source,f'edit:{old}:{index}'),'message_id':mid},edit) for index,parent,edit in rows for old in {cid,*members} for mid in {edit['message_id'],gen_id(source,f'{old}:{parent}')}]
def prepare_edit_lineage(db,r):
    if not r.edit_lineage: return
    def captured(ids):
        paths=captured_edit_paths(db,ids)
        return {key:(route,{path,*([paths[key]] if paths.get(key) else [])}) for key,route,path in db.execute("SELECT file_edit_id,route,path FROM provenance.file_edit_scopes WHERE file_edit_id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps(list(ids))]).fetchall() if route}
    owners=dict(db.execute("SELECT id,cwd FROM conversations WHERE id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[json.dumps(list({m['conversation_id'] for m in r.msgs}))]).fetchall())|{c['id']:c['cwd'] for c in r.convs}
    scopes,cwds=captured({e['id'] for pair in r.edit_lineage for e in pair}),{m['id']:owners.get(m['conversation_id']) for m in r.msgs}
    aliases={key:{e['id'] for pair in pairs for e in pair} for key,pairs in itertools.groupby(sorted(r.edit_lineage,key=lambda p:p[1]['id']),key=lambda p:p[1]['id'])}
    def variants(row,anchor):
        paths,refs,cwd={row['file_path']},{row['id'],*aliases[anchor['id']]},cwds.get(anchor['message_id'])
        while extra:={path for ref in refs if ref in scopes for route,targets in [scopes[ref]] for value in paths if route==os.path.normpath(value) or cwd and route==_resolved(value,cwd) for path in targets if path is not None}-paths: paths|=extra
        return [{**row,'file_path':path} for path in sorted(paths)]
...
def prepare_tool_lineage(db,r,kind="tool_calls",lineage=None):
    ...
    claims,bodies,received=(claims:=[(kind,source,source,'','active') for source in {v[1] for v in lineage}]),(bodies:=typed_logical_rows(db,claims)),{r[0] for r in db.execute("SELECT physical_row_id FROM remote.row_origins WHERE table_name=? AND physical_row_id IN (SELECT json_extract_string(value,'$') FROM json_each(?))",[kind,json.dumps([v[1] for v in claims])]).fetchall()}
...
def prepare_message_lineage(db,r):
    ...
    for c in r.convs:
        if 'convos_message_lineage' in stored.get(c['id'],{}): c['metadata']=json.dumps({**json.loads(c['metadata'] or '{}'),'convos_message_lineage':stored[c['id']]['convos_message_lineage']})
    if not r.message_lineage: return
    protected={v[0] for v in db.execute('SELECT DISTINCT new_source FROM parser_message_lineage').fetchall()}
    current,convs=(current:={m['id']:m for m in r.msgs}),{v['id']:v for v in r.convs}|{row[0]:dict(zip(ARCHIVE_COLUMNS['conversations'],row)) for row in db.execute("SELECT "+','.join(ARCHIVE_COLUMNS['conversations'])+" FROM conversations WHERE id IN (SELECT UNNEST(?))",[list({m['conversation_id'] for m in r.msgs})]).fetchall()}
    for cid in {current[mid]['conversation_id'] for mid,*_ in r.message_lineage if mid in current}:
        conv,session=(conv:=convs[cid]),json.loads(conv['metadata'] or '{}').get('session_id')
...
def parse_claude_code_session(jsonl: Path, bindings=None) -> dict:
    ...
    message_id=lambda idx,i:(bindings or {}).get((src,cid,'message',i),gen_id(src,f'{cid}:{idx}'))
    ...
    turns,(indices)=(turns:=[(idx,i,e,(c:=extract_content(e["message"].get("content",e["message"].get("text","")))),ts_from_iso(e.get("timestamp")),message_id(idx,i)) for idx,(i,e) in enumerate(msg_events)]),({mid:(idx,bool(c['text'] or c['tools'])) for idx,i,e,c,ts,mid in turns})
    ...
    tools,edit_calls,edits=(tools:=[dict(id=gen_id(src,f"tool:{cid}:{t.get('id') or f'{idx}:{j}'}"),message_id=mid,tool_name=t["name"],input=json.dumps(t.get("input",{})),output=json.dumps((tool_results.get(t.get("id")) or {}).get("output","")),status="failed" if (tool_results.get(t.get("id")) or {}).get("error") else "complete" if t.get("id") in tool_results else "pending",duration_ms=None,created_at=ts) for idx,i,e,c,ts,mid in turns for j,t in enumerate(c["tools"]) if "name" in t]),(edit_calls:=[(idx,ts,mid,j,t,tool_results.get(t.get("id"))) for idx,i,e,c,ts,mid in turns for j,t in enumerate(c["tools"]) if t.get("name") in ("Write","Edit","MultiEdit") and t.get("input",{}).get("file_path")]),[dict(id=gen_id(src,f"edit:{cid}:{idx}:{j}"),message_id=mid,file_path=t["input"]["file_path"],edit_type=t["name"].lower(),content=t["input"].get("content") or t["input"].get("new_string",""),created_at=ts,old_content=t["input"].get("old_string")) for idx,ts,mid,j,t,result in edit_calls if result is not None and not result.get("error")]
    evidence,current,lineage=(evidence:=[dict(file_edit_id=gen_id(src,f"edit:{cid}:{idx}:{j}"),status=status,reason=reason,tool_call_id=gen_id(src,f"tool:{cid}:{t.get('id') or f'{idx}:{j}'}")) for idx,ts,mid,j,t,result in edit_calls for status,reason in [(('unknown','result_missing') if result is None else ('invalid','provider_failure') if result.get('error') else ('confirmed','provider_success'))]]),(current:={key:members[0] for key,group in itertools.groupby(sorted(tools,key=lambda t:t['id']),key=lambda t:t['id']) if len(members:=list(group))==1}),parser_tool_lineage([(dict(id=gen_id(src,recipe),message_id=gen_id(src,f'{legacy}:{idx}'),tool_name=t.get('name',t.get('id')),input=t.get('input',{}),output=t['output'] if 'output' in t else {},status='complete' if 'output' in t else 'pending',duration_ms=None,created_at=ts),current[key]) for legacy in {cid,gen_id(src,str(jsonl)),*(bindings or {}).get((src,agent_id or root_session,'legacy'),())} for idx,i,e,c,original,mid in turns for ts in {original,datetime.fromisoformat(e['timestamp'].replace('Z','+00:00')) if e.get('timestamp') else None} for j,t in enumerate(c['tools']) for recipe in (f'tool:{legacy}:{idx}:{j}',f"tool:{legacy}:{t.get('id') or f'{idx}:{j}'}") if (key:=gen_id(src,f"tool:{cid}:{t.get('id') or f'{idx}:{j}'}")) in current]+[({**t,'id':gen_id(src,f'tool:{legacy}:{call}'),'message_id':mid,'created_at':datetime.fromisoformat(e['timestamp'].replace('Z','+00:00')) if e.get('timestamp') else None,'input':json.loads(t['input']),'output':json.loads(t['output'])},t) for legacy in {gen_id(src,str(jsonl)),*(bindings or {}).get((src,agent_id or root_session,'legacy'),())}-{cid} for idx,i,e,c,ts,mid in turns for call in [v.get('id') or f'{idx}:{j}' for j,v in enumerate(c['tools']) if 'name' in v] if (t:=current.get(gen_id(src,f'tool:{cid}:{call}')))],predictive=bool({gen_id(src,str(jsonl)),*(bindings or {}).get((src,agent_id or root_session,'legacy'),())}-{cid}))
    edit_rows={e['id']:e for e in edits}
    edit_lineage=_edit_alias_pairs(src,cid,(bindings or {}).get((src,agent_id or root_session,'legacy'),()),[(f'{idx}:{j}',idx,edit) for idx,ts,mid,j,t,result in edit_calls if result is not None and not result.get('error') for edit in [edit_rows[gen_id(src,f'edit:{cid}:{idx}:{j}')]]])
    return {"conv":dict(id=cid,source=src,title=f"{jsonl.parent.name.replace('-Users-','~/').replace('-','/')} ({jsonl.stem[:8]})",created_at=timestamps[0] if timestamps else None,updated_at=timestamps[-1] if timestamps else None,model=next((m["model"] for m in msgs if m["model"] and m["model"]!="<synthetic>"),None),cwd=system.get("cwd") or next((e.get("cwd") for e in events if e.get("cwd")),None),git_branch=system.get("gitBranch") or next((e.get("gitBranch") for e in events if e.get("gitBranch")),None),project_id=None,metadata=json.dumps({k:v for k,v in {"session_id":agent_id or root_session,"parent_session_id":parent,"session_kind":kind,"session_kind_evidence":"exact" if explicit_agent or sidechain else "inferred","agent_id":agent_id,"agent_name":next((e.get("agentName") for e in events if e.get("agentName")),None),"agent_role":next((e.get("agentType") for e in events if e.get("agentType")),None),"agent_depth":next((e.get("agentDepth") for e in events if e.get("agentDepth") is not None),None),"originator":system.get("entrypoint"),"client_version":system.get("version"),"capture_mode":"transcript","timestamp_basis":"utc"}.items() if v is not None})),"msgs":msgs,"tools":tools,"attachs":[],"edits":edits,"edit_evidence":evidence,"tool_lineage":lineage,"edit_lineage":edit_lineage,"message_lineage":[(mid,idx,(link:=indices.get(uuid2id.get(e.get('parentUuid')),(None,False)))[0],not link[1],gen_id(src,str(jsonl)),jsonl.stem,e.get('timestamp')) for idx,i,e,c,ts,mid in turns if c["text"] or c["tools"]]}
...
def parse_codex_session(jsonl: Path, bindings=None) -> dict | None:
    events,timestamps,legacy_indices,source_times=[],{},{},{}
    for legacy_i,(i,e) in enumerate(iter_jsonl(jsonl)):
        legacy_indices[i]=legacy_i
        if "timestamp" in e: timestamps[i],source_times[i]=ts_from_iso(e["timestamp"]),datetime.fromisoformat(e["timestamp"].replace("Z","+00:00"))
    ...
    message_id=lambda i:(bindings or {}).get((src,cid,'message',i),gen_id(src,f'{cid}:{i}'))
    ...
    def image(i,j,b):
        ...
        return dict(id=gen_id(src,f"attach:{cid}:{i}:{j}"),message_id=message_id(i),filename=f"image-{j+1}.{ext}",mime_type=mime,size=len(body) if body is not None else size,path=str(path) if path else None,url=None if data else url,created_at=timestamps.get(i))
    ...
    if not (msgs := [dict(id=message_id(i), conversation_id=cid, role=p["role"], content=t.strip(), thinking=None, created_at=timestamps.get(i), model=model_at(i), metadata=json.dumps({"provider_index":i}), parent_id=None)
    ...
    current,old_times,old_source_times={key:members[0] for key,group in itertools.groupby(sorted(tools,key=lambda t:t['id']),key=lambda t:t['id']) if len(members:=list(group))==1},list(timestamps.values()),list(source_times.values())
    anchors,lineage=(anchors:=[[legacy_indices[i] for i,p,t in mitems if p['role'] not in ('developer','system') and (images or t)] for images in (False,True)]),parser_tool_lineage([(dict(id=gen_id(src,f"{'tool' if call else 'toolout'}:{legacy}:{old_index}"),message_id=gen_id(src,f"{legacy}:{parent}"),tool_name=p['name'] if call else p.get('call_id'),input=norm_args(p) if call else {},output={} if call else p.get('output',''),status='pending' if call else 'complete',duration_ms=None,created_at=ts),current[key]) for legacy in {cid,gen_id(src,str(jsonl)),*(bindings or {}).get((src,provider_id,'legacy'),())} for i,p in items if p.get('type') in ('function_call','function_call_output') for call,old_index,key in [(p['type']=='function_call',legacy_indices[i],gen_id(src,f"tool:{cid}:{p.get('call_id') or i}"))] if key in current for parent in {old_index,*[next((j for j in reversed(indices) if j<=old_index),indices[0]) for indices in anchors if indices]} for ts in {timestamps.get(i),source_times.get(i),old_times[old_index] if old_index<len(old_times) else None,old_source_times[old_index] if old_index<len(old_source_times) else None}]+[({**t,'id':gen_id(src,recipe),'created_at':source_times.get(i),'input':json.loads(t['input']),'output':json.loads(t['output'])},t) for legacy in (bindings or {}).get((src,provider_id,'legacy'),()) if legacy!=cid for i,p in items for prefix,key in ([('tool',p.get('call_id') or i)] if p.get('type')=='function_call' else [('custom',i)] if p.get('type')=='custom_tool_call' else []) for recipe in [f'{prefix}:{legacy}:{key}'] if (t:=current.get(gen_id(src,f'{prefix}:{cid}:{key}')))],predictive=bool({gen_id(src,str(jsonl)),*(bindings or {}).get((src,provider_id,'legacy'),())}-{cid}))
    edit_rows={e['id']:e for e in edits}
    edit_lineage=_edit_alias_pairs(src,cid,(bindings or {}).get((src,provider_id,'legacy'),()),[(f'{i}:{j}',next((index for index,_,_ in reversed(mitems) if index<=i),mitems[0][0]),edit_rows[gen_id(src,f'edit:{cid}:{i}:{j}')]) for i,p,tool,(status,reason),rows in candidates if status=='confirmed' for j,row in enumerate(rows)])
    return {"conv":dict(id=cid,source=src,title=(bindings or {}).get((src,cid,'title'),meta.get("cwd") or jsonl.stem),created_at=min(timestamps.values(),default=None),updated_at=max(timestamps.values(),default=None),model=next((m["model"] for m in msgs if m["role"]=="assistant" and m["model"]),None),cwd=meta.get("cwd"),git_branch=(meta.get("git") or {}).get("branch"),project_id=None,metadata=json.dumps({k:v for k,v in {"session_id":provider_id,"parent_session_id":spawn.get("parent_thread_id") or meta.get("parent_thread_id"),"session_kind":"subagent" if subagent is not None else "main","session_kind_evidence":"exact" if subagent is not None else "inferred","agent_name":spawn.get("agent_nickname") or meta.get("agent_nickname"),"agent_role":spawn.get("agent_role") or (subagent if isinstance(subagent,str) else meta.get("agent_role")),"agent_depth":spawn.get("depth"),"originator":meta.get("originator"),"client_version":meta.get("cli_version"),"capture_mode":"transcript","timestamp_basis":"utc","git_repository":(meta.get("git") or {}).get("repository_url"),"git_commit":(meta.get("git") or {}).get("commit_hash"),"forked_from_id":meta.get("forked_from_id"),"thread_source":meta.get("thread_source")}.items() if v is not None})),"msgs":msgs,"tools":tools,"attachs":[image(i,j,b) for i,p,t in mitems for j,b in enumerate(x for x in p.get("content",[]) if isinstance(x,dict) and x.get("type")=="input_image")],"edits":edits,"edit_evidence":evidence,"tool_lineage":lineage,"edit_lineage":edit_lineage,"message_lineage":[(message_id(i),legacy_indices[i],None,False,gen_id(src,str(jsonl)),jsonl.stem,old_source_times[legacy_indices[i]].isoformat() if legacy_indices[i]<len(old_source_times) else None) for i,p,t in mitems if t and p['role'] not in ('developer','system')]}
...
def upsert(conn, r: ParseResult):
    ...
    def replace_preserving(table, rows):
        ...
        with preserve_fact_heads(conn,[(table,values[0]) for values,previous in changed if previous],observed=True):
    ...
    if delta:=[v for v in incoming if evidence.get(v["file_edit_id"])!=evidence_row(v)]:
        with preserve_fact_heads(conn,[("edit.observed",v["file_edit_id"]) for v in delta]): conn.executemany("INSERT OR REPLACE INTO provenance.file_edit_evidence VALUES (?,?,?,?)",list(map(evidence_row,delta)))
    ...
    for kind,children in (('tool_calls',r.tools),('file_edits',r.edits)): (record_tool_lineage(conn,[(mid,'',True) for mid in changed_rows],kind),reconcile_tool_lineage(conn,changed_rows,[t['id'] for t in children],kind))
    (record_tool_lineage(conn,[(cid,'',True) for cid in changed_convs],"messages"),reconcile_tool_lineage(conn,changed_convs,mids,"messages"),retire_parser_rows(conn))
...
def hook_result(source,path,bindings=None):
    ...
    return ParseResult(convs=[session["conv"]],msgs=session["msgs"],tools=session["tools"],attachs=session["attachs"],edits=session["edits"],edit_evidence=session["edit_evidence"],tool_lineage=session["tool_lineage"],edit_lineage=session["edit_lineage"],message_lineage=session["message_lineage"]) if session else ParseResult()
