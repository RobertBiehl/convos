...
def record_tool_lineage(db,messages,kind="tool_calls",metadata=None):
    label,owner=("tool","messages") if kind=="tool_calls" else ("message","conversations")
...
def reconcile_tool_lineage(db,messages=(),tools=(),kind="tool_calls"):
    label="tool" if kind=="tool_calls" else "message"
...
def rebuild_parser_lineage(db):
    for kind,owner,label in (('tool_calls','messages','tool'),('messages','conversations','message')):
    ...
    (restore_parser_parents(db),retire_parser_rows(db))
def record_parser_replicas(db,items):
    for kind,owner in (('tool_calls','messages'),('messages','conversations')):
...
def retire_parser_rows(db):
    ...
    for kind,label in (('tool_calls','tool'),('messages','message'),('conversations','conversation')):
        ...
        for at in range(0,len(plans),500):
            ...
            if selected:
                with preserve_fact_heads(db,[(kind,p[0]) for p,body in selected]): (_insert_pages(db,'parser_retired_rows',[(kind,p[0],p[2],p[3],p[4],body,{k:v for k,v in zip(ARCHIVE_COLUMNS[kind],raws[p[0]]) if k not in ROW_FIELDS_V1[kind] or k in dict(ARCHIVE_FKS.get(kind,()))},p[1]) for p,body in selected],mode=' OR IGNORE'),db.execute(f"DELETE FROM {kind} WHERE id IN (SELECT UNNEST(?))",[[p[0] for p,body in selected]]),kind=='conversations' and db.executemany("UPDATE provider_sessions SET conversation_id=? WHERE conversation_id=?",[(p[1],p[0]) for p,body in selected]),_archive_touch(db,[(kind,p[0]) for p,body in selected]))
...
def restore_retired_proofs(db): return _insert_pages(db,'remote.row_conflicts',[(pid,body) for pid,expected,raw in db.execute("SELECT p.id,p.content_hash,r.body FROM parser_retired_rows r JOIN remote.row_proofs p ON (p.row_kind,p.source_row_id)=(r.kind,r.source) AND (r.author='' OR r.author=p.author_user_id) WHERE NOT EXISTS(SELECT 1 FROM remote.row_conflicts c WHERE c.proof_id=p.id)").fetchall() if (body:=matching_logical_row(json.loads(raw),expected)) is not None],mode=' OR IGNORE')
...
def parser_tool_lineage(pairs): return sorted({(new['message_id'],old['id'],provenance_digest(parser_logical_row('tool_calls',{k:json.dumps(v) if k in ROW_JSON_V1 else v for k,v in old.items()})),new['id'],provenance_digest(parser_logical_row('tool_calls',new))) for old,new in pairs if old['id']!=new['id']})
def prepare_tool_lineage(db,r):
    if not r.tool_lineage: return
    claims,bodies,received=(claims:=[('tool_calls',source,source,'','active') for source in {v[1] for v in r.tool_lineage}]),(bodies:=typed_logical_rows(db,claims)),{r[0] for r in db.execute("SELECT physical_row_id FROM remote.row_origins WHERE table_name='tool_calls' AND physical_row_id IN (SELECT UNNEST(?))",[[v[1] for v in claims]]).fetchall()}
    kept=[(mid,dict(zip(('old_id','old_hash','current_id','current_hash'),value))) for mid,*value in r.tool_lineage if value[0] not in received and (body:=bodies.get(('tool_calls',value[0],value[0],'','active'))) is not None and provenance_digest(body)==value[1]]
    for m in r.msgs:
        if records:=[value for mid,value in kept if mid==m['id']]: m['metadata']=json.dumps({**json.loads(m['metadata'] or '{}'),'convos_tool_lineage':{'v':1,'records':records}})
...
def upsert(conn, r: ParseResult):
    ...
    (_parse_result_refs(conn,r),prepare_tool_lineage(conn,r),prepare_message_lineage(conn,r),_quarantine_stubs(r))
    ...
    previous_tools=_rows_by_id(conn,"tool_calls",[t["id"] for t in r.tools])
    def replace_preserving(table, rows):
        if not rows: return [],0
        old,skip,payload,canonical,changed,histories=(old:=_rows_by_id(conn,table,[r["id"] for r in rows])),(skip:={"tool_calls":7,"attachments":7,"artifacts":6,"file_edits":5}[table]),(payload:=lambda values:tuple(v for i,v in enumerate(values) if i not in (0,skip))),(canonical:=lambda values:payload([json.dumps(json.loads(v),sort_keys=True,separators=(",",":"),ensure_ascii=True,allow_nan=False) if table=="tool_calls" and i in (3,4) and isinstance(v,str) else v for i,v in enumerate(values)])),(changed:=[(list(row.values()),old.get(row["id"])) for row in rows if not old.get(row["id"]) or canonical(old[row["id"]])!=canonical(tuple(row.values()))]),[_history_row(table,previous,payload(previous)) for values,previous in changed if previous]
        with preserve_fact_heads(conn,[(table,values[0]) for values,previous in changed if previous],observed=True):
            ...
            if table=='file_edits' and histories:
                ...
                for history,previous in zip(histories,[previous for values,previous in changed if previous]):
    ...
    (record_tool_lineage(conn,[(mid,'',True) for mid in changed_rows]),reconcile_tool_lineage(conn,changed_rows,[t['id'] for t in r.tools]))
