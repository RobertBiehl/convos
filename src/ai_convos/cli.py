...
CREATE TABLE IF NOT EXISTS provenance.file_edit_scopes(file_edit_id VARCHAR PRIMARY KEY,path VARCHAR,repository VARCHAR,root VARCHAR,checkout VARCHAR,observed_at TIMESTAMP);
...
def _repository(root): return (lambda root,roots,remotes,lineage:dict(id=provenance_digest({"lineage":lineage,"remotes":remotes}) if lineage else provenance_digest({"local":str(root)}),lineage=lineage,root=str(root),roots=roots,remotes=remotes,checkout=_checkout(root)))(root:=Path(root),roots:=sorted(_git_maybe(root,"rev-list","--max-parents=0","HEAD").decode().split()),remotes:=_git_remotes(root),provenance_digest({"git_roots":roots}) if roots else None)
def repository_state(db): return {"roots":dict(db.execute("SELECT root,repository FROM provenance.repository_checkouts").fetchall()),"checkouts":dict(db.execute("SELECT id,repository FROM provenance.repository_checkouts").fetchall()),"checkout_roots":dict(db.execute("SELECT id,root FROM provenance.repository_checkouts").fetchall()),"aliases":dict(db.execute("SELECT evidence,CASE WHEN COUNT(DISTINCT repository)=1 THEN MIN(repository) END FROM provenance.repository_aliases GROUP BY evidence").fetchall())}
def _refresh_repository(): (_git_root.cache_clear(),_repository.cache_clear())
def repository(path,known=None,refresh=True): return (lambda value,state,evidence,resolved:{**value,"id":resolved or value["id"],"alias":None if resolved or not evidence else evidence})(value:={**_repository(str(root)),"head":_git_maybe(root,"rev-parse","--verify","HEAD").decode().strip(),"branch":_git_maybe(root,"symbolic-ref","--short","HEAD").decode().strip()},state:=repository_state(known) if known is not None and hasattr(known,"execute") else known or {"roots":{},"checkouts":{},"checkout_roots":{},"aliases":{}},evidence:=repository_evidence(value),state["checkouts"].get(value["checkout"]) or evidence and state["aliases"].get(evidence)) if (not refresh or _refresh_repository() is None) and (root:=_git_root(Path(path))) else None
...
def snapshot_edit_scopes(edits,known=None):
    ...
    cache={}; out=[]
    for e in edits:
        repo,path,_=_provenance_where(e["path"],e["cwd"],cache,known)
        out.append((e["id"],path,repo["id"] if repo else None,repo["root"] if repo else None,repo["checkout"] if repo else None,captured))
    return out
...
def capture_provenance(path=None,edit_ids=None,source="sync"):
    ...
    try:
        edits=_provenance_edits(core,edit_ids)
        known=repository_state(core)
        ...
        conversations=core.execute("SELECT conversation,cwd,repository,root,checkout,CAST(observed_at AS VARCHAR) FROM provenance.conversation_scopes WHERE repository IS NOT NULL AND repository NOT IN (SELECT id FROM provenance.repositories)").fetchall()
    ...
    scopes=snapshot_scopes(missing,known)
    conversations+=scopes
    edit_scopes=snapshot_edit_scopes([e for e in edits if e["scope_path"] is None],known)
    frozen={r[0]:r for r in edit_scopes}
    for e in edits:
        if e["id"] in frozen: _,e["scope_path"],e["repository"],e["root"],e["checkout"],e["scope_at"]=frozen[e["id"]]
    ...
    stale=[root for root in known["roots"] if str(_git_root(root))!=root]
    ...
    with contextlib.closing(core),_transaction(core):
        ...
        scopes and core.executemany("INSERT OR IGNORE INTO provenance.conversation_scopes VALUES (?,?,?,?,?,?)",scopes)
        edit_scopes and core.executemany("INSERT OR IGNORE INTO provenance.file_edit_scopes VALUES (?,?,?,?,?,?)",edit_scopes)
...
def init_schema(conn):
    ...
    if conn.execute("SELECT version FROM core_schema WHERE singleton").fetchone()[0]<3:
        aliases=[(r[0],repository_evidence({"lineage":r[1],"remotes":json.loads(r[2])})) for r in conn.execute("SELECT id,lineage,CAST(remotes AS VARCHAR) FROM provenance.repositories WHERE lineage IS NOT NULL").fetchall()]
        legacy_edits=[(r[0],f"external/{provenance_digest(r[0])[:24]}/unknown",None,None,None,None) for r in conn.execute("SELECT id FROM file_edits fe WHERE NOT EXISTS (SELECT 1 FROM remote.row_origins o WHERE o.table_name='file_edits' AND o.physical_row_id=fe.id)").fetchall()]
        with _transaction(conn):
            conn.execute("CREATE TABLE provenance.file_edit_files_v3(file_edit_id VARCHAR PRIMARY KEY,file_id VARCHAR,old_content_hash VARCHAR,new_content_hash VARCHAR,evidence VARCHAR); INSERT INTO provenance.file_edit_files_v3 SELECT * FROM provenance.file_edit_files; DROP TABLE provenance.file_edit_files; ALTER TABLE provenance.file_edit_files_v3 RENAME TO file_edit_files")
            aliases and conn.executemany("INSERT OR IGNORE INTO provenance.repository_aliases VALUES (?,?)",aliases)
            legacy_edits and conn.executemany("INSERT OR IGNORE INTO provenance.file_edit_scopes VALUES (?,?,?,?,?,?)",legacy_edits)
...
def upsert(conn, r: ParseResult):
    ...
    frozen=frozen if frozen is not None else [(c["id"],str(Path(c["cwd"]).expanduser().resolve()) if c["cwd"] else None,None,None,None,datetime.now(timezone.utc)) for c in r.convs]
    ...
    getattr(r,"edit_scopes",None) and conn.executemany("INSERT OR IGNORE INTO provenance.file_edit_scopes VALUES (?,?,?,?,?,?)",r.edit_scopes)
    ...
    with (HOOK_DIR/".lock").open("w") as lock:
        ...
        for q in HOOK_DIR.glob("*.json"):
            ...
            try:
                ...
                conn = get_db()
                init_schema(conn)
                known=repository_state(conn)
                conn.close()
                r.scopes=snapshot_scopes([(c["id"],c["cwd"]) for c in r.convs],known)
                r.edit_scopes=snapshot_edit_scopes(edit_scope_inputs(r),known)
                conn=get_db()
...
def sync(watch: bool = typer.Option(False, "-w"), interval: int = typer.Option(300, "-i"), claude_code: bool = True, codex: bool = True, full: bool = typer.Option(False, "--full", help="Re-parse/re-fetch everything, ignoring incremental state"), verbose: bool = typer.Option(False, "-v", "--verbose"), local_only: bool = typer.Option(False, "--local-only", help="Import local agent sessions and configured exports without contacting web sources.")):
    ...
    def do_sync():
        ...
        git_known=repository_state(conn)
        conn.close()
            ...
            with ThreadPoolExecutor(max_workers=min(4, len(jobs))) as ex:
                ...
                for fut in as_completed(futs):
                    ...
                    if saved := j.get("saved"):
                        ...
                        with (HOOK_DIR/".lock").open("w") as lock:
                            fcntl.flock(lock, fcntl.LOCK_EX)
                            r.scopes=snapshot_scopes([(c["id"],c["cwd"]) for c in r.convs],git_known)
                            r.edit_scopes=snapshot_edit_scopes(edit_scope_inputs(r),git_known)
                            conn = get_db()
