...
def detect_source(path: Path): return "muse" if path.is_dir() and (path/"session.jsonl").exists() and "runtime.session" in open(path/"session.jsonl").read(4000) else "codex" if path.is_dir() and (path/"sessions").exists() else "claude-code" if path.is_dir() else "chatgpt" if path.suffix==".zip" or "chatgpt" in path.name.lower() else "claude" if "chat_messages" in (data:=required(json.loads(path.read_text()),ValueError(f"Empty export: {path}")))[0] else "chatgpt"
...
CREATE OR REPLACE VIEW parser_tool_history AS SELECT l.old_id,min(l.new_id) current_id FROM parser_tool_lineage l JOIN messages m ON m.id=l.message_id JOIN tool_calls old ON old.id=l.old_id JOIN tool_calls new ON new.id=l.new_id GROUP BY l.old_id HAVING count(DISTINCT l.new_id)=1;
...
def _git_root(path):
    ...
    except subprocess.CalledProcessError as e: return None if b"not a git repository" in e.stderr.lower() else (_ for _ in ()).throw(e)
...
def capture_provenance(path=None,edit_ids=None,conversation_ids=None,source="sync"):
    ...
    with _core(path,True,purpose="provenance.plan") as core:
        queued=core.execute("SELECT kind,entity FROM provenance.pending ORDER BY generation,kind,entity"+(" LIMIT 500" if targeted else "")).fetchall()
        eids,cids=sorted(set(eids)|{entity for kind,entity in queued if kind=='file_edits'}),sorted(set(cids)|{entity for kind,entity in queued if kind=='conversations'})
    ...
    with _core(path,purpose="provenance.commit") as core,_transaction(core):
        required(generations==_provenance_generations(core,eids,cids),ValueError("Provenance inputs changed during observation; committed conversations are preserved and provenance will retry."))
...
def project_archive_rows(db,table,columns,rows,preserve=True):
    ...
    with preserve_fact_heads(db,[(table,row_id) for row_id in ids if row_id in occupied],observed=[(table,v[0]) for v,o in rows if not o]) if preserve else contextlib.nullcontext():
        ...
        if origins: (_insert_pages(db,"remote.row_origins",origins,mode=" OR REPLACE"),db.execute("DELETE FROM remote.row_references r USING remote.row_origins o WHERE (r.table_name,r.physical_row_id,r.author_user_id,r.source_row_id)=(o.table_name,o.physical_row_id,o.author_user_id,o.source_row_id) AND r.table_name=? AND r.physical_row_id IN (SELECT UNNEST(?))",[table,ids]))
        if table in ("file_edits","tool_calls"): _apply_signed_edit_evidence(db,[v[0] for v in values] if table=="file_edits" else (),[v[0] for v in values] if table=="tool_calls" else ())
    if table=='messages': record_tool_lineage(db,[(v[0],o['author_user_id'] if o else '',not o) for v,o in rows])
    if table in ('messages','tool_calls'): reconcile_tool_lineage(db,ids if table=='messages' else (),[o['source_row_id'] if o else v[0] for v,o in rows] if table=='tool_calls' else ())
...
def record_tool_lineage(db,messages):
    if not messages: return
    metadata,records=(metadata:={mid:json.loads(raw or '{}') for mid,raw in db.execute("SELECT id,metadata FROM messages WHERE id IN (SELECT UNNEST(?))",[[mid for mid,author,native in messages]]).fetchall()}),[(mid,author,native,r['old_id'],r['old_hash'],r['current_id'],r['current_hash'],None,None) for mid,author,native in messages for meta in [metadata.get(mid)] if isinstance(meta,dict) and isinstance(meta.get('convos_tool_lineage'),dict) and type(meta['convos_tool_lineage'].get('v')) is int and meta['convos_tool_lineage']['v']==1 and isinstance(meta['convos_tool_lineage'].get('records'),list) for r in meta['convos_tool_lineage']['records'] if isinstance(r,dict) and set(r)=={'old_id','old_hash','current_id','current_hash'} and all(isinstance(r[k],str) and re.fullmatch('[0-9a-f]{'+str(64 if k.endswith('hash') else 16)+'}',r[k]) for k in r) and r['old_id']!=r['current_id']]
    return (db.execute("DELETE FROM parser_tool_lineage WHERE message_id IN (SELECT UNNEST(?))",[[mid for mid,author,native in messages]]),_insert_pages(db,"parser_tool_lineage",records,mode=" OR IGNORE"))[-1]
def reconcile_tool_lineage(db,messages=(),tools=()):
    if not messages and not tools: return
    rows=db.execute("SELECT * FROM parser_tool_lineage WHERE message_id IN (SELECT UNNEST(?)) OR old_source IN (SELECT UNNEST(?)) OR new_source IN (SELECT UNNEST(?))",[list(messages),list(tools),list(tools)]).fetchall()
    for author,native in {(r[1],r[2]) for r in rows}:
        for at in range(0,len(selected:=[r for r in rows if (r[1],r[2])==(author,native)]),500):
            page,sources,found=(page:=selected[at:at+500]),(sources:={r[i] for r in page for i in (3,5)}),db.execute("SELECT t.id,coalesce(o.source_row_id,t.id) FROM tool_calls t LEFT JOIN remote.row_origins o ON o.table_name='tool_calls' AND o.physical_row_id=t.id WHERE o.author_user_id=? AND o.source_row_id IN (SELECT UNNEST(?)) OR ? AND o.physical_row_id IS NULL AND t.id IN (SELECT UNNEST(?))",[author,list(sources),native,list(sources)]).fetchall()
            bindings,claims,bodies=(bindings:={source:members[0][0] for source,group in itertools.groupby(sorted(found,key=lambda r:r[1]),key=lambda r:r[1]) if len(members:=list(group))==1}),(claims:=[('tool_calls',physical,source,author,'active') for source,physical in bindings.items()]),{claim[2]:value for claim,value in typed_logical_rows(db,claims).items()}
            if changes:=[((*((bindings[r[3]],bindings[r[5]]) if r[3] in bodies and r[5] in bodies and provenance_digest(bodies[r[3]])==r[4] and provenance_digest(bodies[r[5]])==r[6] else (None,None)),r[0],author,r[3],r[4],r[5],r[6])) for r in page]: db.executemany("UPDATE parser_tool_lineage SET old_id=?,new_id=? WHERE message_id=? AND author=? AND old_source=? AND old_hash=? AND new_source=? AND new_hash=?",changes)
...
def project_logical_rows(db,items,defer=False):
    ...
    with preserve_fact_heads(db,[(table,mapped("file_edits",source) if table=="edit.observed" else source if table=="repository.observed" else physical) for row,p,pid,native,*maps in items for table,source,mapped,physical,origin in [_logical_parts(row,p,pid,native,maps[0] if maps else None)] if table in ("repository.observed","edit.observed","file_edits") or native and table in ARCHIVE_COLUMNS]):
        ...
        (out.extend((item[0]["kind"],physical) for item,physical in facts),_insert_pages(db,"remote.provenance_origins",[(item[0]["kind"],physical,item[1]["workspace"],item[1]["author_user_id"],item[0]["id"],item[2]) for item,physical in facts if item[0]["kind"] in PROVENANCE_KINDS],mode=" OR REPLACE"))
        if defer: (_retain_lossy_replicas(db,items,defer),[repair_parent_links(db,author,parents) for author in authors])
    (record_tool_lineage(db,[(physical,p['author_user_id'],native or defer and p['author_user_id'] in authors) for row,p,pid,native,*maps in items if row['kind']=='messages' for physical in [row['id'] if native else remote_id(p['author_user_id'],'messages',row['id'])]]),reconcile_tool_lineage(db,[physical for table,physical in out if table=='messages'],[row['id'] for row,p,pid,native,*maps in items if row['kind']=='tool_calls']))
...
def typed_logical_rows(db,claims,references=True,historical=False):
    ...
    facts={(r["kind"],r["entity"]):r for r in provenance_records(db,{(kind,physical) for kind,physical,source,user,state in claims if kind in PROVENANCE_KINDS},historical)}
    mapped,out=logical_references(db,{raw[cols.index(column)] for (kind,physical),(cols,raw) in found.items() for column,parent in ARCHIVE_FKS.get(kind,()) if raw[cols.index(column)]}|{record["payload"][field] for record in facts.values() for field in ("turn","edit") if field in record["payload"]},references),{}
    for claim in claims:
...
def _backup_attachments(conn,source,backup,database_hash):
    ...
    for ref,body_hash,size,path in rows:
        candidates=paths.get(body_hash,[])+([Path(path)] if path and not body_hash else [])+([source.parent/"attachments"/body_hash] if body_hash else [])
        ...
        if {"remote.row_proofs","remote.row_origins","remote.provenance_origins"}<=available:
            with contextlib.closing(donor.cursor()) as reader:
                ...
                while proofs:=reader.fetchmany(page):
                    ...
                    found,paths=typed_logical_rows(donor,claims,"remote.row_references" in available,historical=True),captured_edit_paths(donor,[physical for kind,physical,source,user,state in claims if kind=="file_edits"])
...
class ParseResult:
    ...
    def __iadd__(self,other): return ([getattr(self,name).extend(getattr(other,name)) for name in ("convs","msgs","tools","attachs","artifacts","edits","edit_evidence","tool_lineage","failed_inputs")],self)[-1]
    def __add__(self,other): return ParseResult(**{name:[*getattr(self,name),*getattr(other,name)] for name in ("convs","msgs","tools","attachs","artifacts","edits","edit_evidence","tool_lineage","failed_inputs")})
...
def parse_source(path: Path, source: str|None = None, bindings=None) -> ParseResult:
    parsers,src={"chatgpt":parse_chatgpt,"claude":parse_claude,"claude-code":parse_claude_code,"codex":parse_codex,"muse":parse_muse},source or detect_source(path)
    return (required(src in parsers,ValueError(f"Unknown source: {src}")),parsers[src](path,bindings=bindings) if src in ("claude-code","codex","muse") else parsers[src](path))[1]
...
def parser_tool_lineage(pairs): return sorted({(new['message_id'],old['id'],provenance_digest(logical_row('tool_calls',list(old),[json.dumps(v) if k in ROW_JSON_V1 else v for k,v in old.items()])),new['id'],provenance_digest(logical_row('tool_calls',list(new),list(new.values())))) for old,new in pairs if old['id']!=new['id']})
...
def parse_codex_session(jsonl: Path, bindings=None) -> dict | None:
    events,timestamps,legacy_indices=[],{},{}
    ...
    if not (msgs := [dict(id=gen_id(src, f"{cid}:{i}"), conversation_id=cid, role=p["role"], content=t.strip(), thinking=None, created_at=timestamps.get(i), model=model_at(i), metadata=json.dumps({"provider_index":i}), parent_id=None) for i, p, t in mitems]): return None
    ...
    def patch_edits(args):
        ...
        def hunk(text): return ([line[1:] if line.startswith((" ","-")) else line for line in text.splitlines() if not line.startswith(("***","+"))],[line[1:] if line.startswith((" ","+")) else line for line in text.splitlines() if not line.startswith(("***","-"))])
...
def parse_muse_session(jsonl: Path, bindings=None) -> dict | None:
    recs=[(i,r) for i,r in iter_jsonl(jsonl)]
    required(sum(1 for line in jsonl.read_text().splitlines() if line.strip())==len(recs),ValueError(f"Malformed Muse session: {jsonl}"))
    envs=sorted([e for _,r in recs for e in ([r] if "children" not in r else [json.loads(c["record_json"]) for c in r["children"]])],key=lambda e:e.get("sequence",0))
    sub,sid=(len(jsonl.parts)>=4 and jsonl.parts[-3]=="subagent" and jsonl.parts[-1]=="session.jsonl"),next((s for e in envs if (s:=(e.get("stream") or {}).get("id"))),None)
    cid=(bindings or {}).get(("muse",sid),gen_id("muse",str(jsonl)))
    runs,calls,outs,fails,sums,txts,models=[(e.get("sequence",0),e["payload"]["run_id"],e["payload"]["event"]["prompt"],ts_from_epoch(e["recorded_at"]/1e6) if e.get("recorded_at") else None) for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="started" and e["payload"]["event"].get("prompt") and e["payload"].get("run_id")],[(e.get("sequence",0),e["payload"]["event"]["response_id"],e["payload"]["event"]["message_id"],t,ts_from_epoch(e["recorded_at"]/1e6) if e.get("recorded_at") else None) for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="assistant_tool_calls_committed" and e["payload"]["event"].get("response_id") and e["payload"]["event"].get("message_id") for t in e["payload"]["event"].get("tool_calls",[]) or []],{r["tool_call_id"]:r.get("text","") for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="tool_result_batch_committed" for r in e["payload"]["event"].get("results",[]) or [] if r.get("tool_call_id")},{c for e in envs if e.get("payload_type")=="tool_batch.effect.terminal" and (rec:=((e.get("payload") or {}).get("record") or {})) and (c:=rec.get("call_id")) and (((rec.get("outcome") or {}).get("task_completion") or {}).get("terminal") or {}).get("kind")=="failed"},{e["payload"]["event"]["message_id"]:(e.get("sequence",0),ts_from_epoch(e["recorded_at"]/1e6) if e.get("recorded_at") else None,e["payload"]["event"].get("text","")) for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="reasoning_summary_committed" and e["payload"]["event"].get("message_id")},{e["payload"]["event"]["message_id"]:(e.get("sequence",0),ts_from_epoch(e["recorded_at"]/1e6) if e.get("recorded_at") else None,e["payload"]["event"].get("text","")) for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="assistant_message_committed" and e["payload"]["event"].get("message_id")},[(e.get("sequence",0),e["payload"]["event"]["model"]) for e in envs if (e.get("payload") or {}).get("kind")=="run" and isinstance((e.get("payload") or {}).get("event"),dict) and e["payload"]["event"].get("kind")=="model_completed" and e["payload"]["event"].get("model")]
    model_at=lambda s: next((m for q,m in reversed(models) if q<=s),None)
    batches,order,bounds={m:(s,t) for s,r,m,_,t in calls},{s:i for i,s in enumerate(sorted({e.get("sequence",0) for e in envs}))},[ts_from_epoch(e["recorded_at"]/1e6) for e in envs if e.get("recorded_at")]
    cands=[(r,m,t,ts,rows,status,reason) for _,r,m,t,ts in calls if t.get("name") in ("write_file","edit_file") and isinstance((a:=json.loads(t.get("args")) if isinstance(t.get("args"),str) else t.get("args") or {}),dict) for rows in [[(a["path"],{"write_file":"write","edit_file":"edit"}[t["name"]],a.get("content") if t["name"]=="write_file" else a.get("replace"),None if t["name"]=="write_file" else a.get("find"))] if a.get("path") else []] for txt in [outs.get(t.get("call_id"),"")] for status,reason in [(("invalid","provider_failure") if t.get("call_id") in fails else ("confirmed","provider_success") if isinstance(txt,str) and txt.startswith({"write_file":"wrote ","edit_file":"edited"}[t["name"]]) else ("unknown","result_missing" if t.get("call_id") not in outs else "unconfirmed_result"))]]
    edits,evidence=[dict(id=gen_id("muse",f"edit:{cid}:{t.get('call_id') or t['id']}"),message_id=gen_id("muse",f"{cid}:{m}"),file_path=fp,edit_type=op,content=c,created_at=ts,old_content=o) for r,m,t,ts,rows,status,reason in cands if status=="confirmed" for fp,op,c,o in rows],[dict(file_edit_id=gen_id("muse",f"edit:{cid}:{t.get('call_id') or t['id']}"),status=status,reason=reason,tool_call_id=gen_id("muse",f"tool:{cid}:{t.get('call_id') or t['id']}")) for r,m,t,ts,rows,status,reason in cands]
    umsgs,amsgs,atexts,asumms,tools=[dict(id=gen_id("muse",f"{cid}:{i}"),conversation_id=cid,role="user",content=p,thinking=None,created_at=t,model=None,metadata=json.dumps({"provider_index":order[q]}),parent_id=None) for q,i,p,t in runs],[dict(id=gen_id("muse",f"{cid}:{m}"),conversation_id=cid,role="assistant",content=txts.get(m,(0,None,""))[2],thinking=None,created_at=t,model=model_at(q),metadata=json.dumps({"provider_index":order[q]}),parent_id=None) for m,(q,t) in batches.items()],[dict(id=gen_id("muse",f"{cid}:{s}"),conversation_id=cid,role="assistant",content=t,thinking=None,created_at=ts,model=model_at(q),metadata=json.dumps({"provider_index":order[q]}),parent_id=None) for s,(q,ts,t) in txts.items()],[dict(id=gen_id("muse",f"{cid}:{s}"),conversation_id=cid,role="assistant",content="",thinking=t,created_at=ts,model=model_at(q),metadata=json.dumps({"provider_index":order[q]}),parent_id=None) for s,(q,ts,t) in sums.items()],[dict(id=gen_id("muse",f"tool:{cid}:{t.get('call_id') or t['id']}"),message_id=gen_id("muse",f"{cid}:{m}"),tool_name=t["name"],input=t["args"] if isinstance(t.get("args"),str) else json.dumps(t.get("args",{})),output=json.dumps(outs.get(t.get("call_id"),"")),status="failed" if t.get("call_id") in fails else "complete" if t.get("call_id") in outs else "pending",duration_ms=None,created_at=ts) for _,r,m,t,ts in calls]
    cwd,branch,commit=next((r.get("cwd") for e in envs if e.get("payload_type")=="runtime.session.route_facts" and isinstance((r:=(e.get("payload") or {}).get("record") or {}),dict) and r.get("cwd")),None),next(((((e.get("payload") or {}).get("record") or {}).get("reference") or {}).get("name") for e in envs if e.get("payload_type")=="session.workspace_branch.observed"),None),next((r.get("commit") for e in envs if e.get("payload_type")=="session.workspace_branch.observed" and isinstance((r:=(e.get("payload") or {}).get("record") or {}),dict) and r.get("commit")),None)
    return {"conv":dict(id=cid,source="muse",title=(runs[0][2][:80] if runs else f"muse ({(sid or jsonl.parent.name)[:8]})"),created_at=min(bounds) if bounds else None,updated_at=max(bounds) if bounds else None,model=models[0][1] if models else None,cwd=cwd,git_branch=branch,project_id=None,metadata=json.dumps({k:v for k,v in {"session_id":sid,"parent_session_id":jsonl.parts[-4] if sub and re.fullmatch(r"[0-9a-f-]{1,64}",jsonl.parts[-4]) else None,"session_kind":"subagent" if sub else "main","session_kind_evidence":"exact" if sub else "inferred","capture_mode":"transcript","git_commit":commit}.items() if v is not None})),"msgs":umsgs+amsgs+atexts+asumms,"tools":tools,"attachs":[],"edits":edits,"edit_evidence":evidence,"tool_lineage":[]} if umsgs+amsgs+atexts+asumms else None
def parse_muse(muse_dir: Path, files: list[Path] | None = None, bindings=None) -> ParseResult: return _parse_sessions(files or muse_dir.rglob("session.jsonl"),parse_muse_session,bindings)
...
def upsert(conn, r: ParseResult):
    ...
    (_parse_result_refs(conn,r),prepare_tool_lineage(conn,r),_quarantine_stubs(r))
    ...
    if frozen:=r.scopes if r.scopes is not None else pending_scopes([(c["id"],c["cwd"]) for c in r.convs]): conn.executemany("INSERT OR IGNORE INTO provenance.conversation_scopes VALUES (?,?,?,?,?,?)",frozen)
...
def _muse_session_path(root,sid): return next((p.resolve() for p in sorted(root.rglob(f"{sid}/session.jsonl")) if p.is_file()),None) if re.fullmatch(r"[0-9a-f-]{1,64}",sid or "") else None
def hook_root(source): return Path(os.environ.get("CLAUDE_CONFIG_DIR", Path.home()/".claude"))/"projects" if source == "claude-code" else Path(os.environ.get("MUSE_HOME",Path.home()/".local/share/muse"))/"sessions" if source=="muse" else Path(os.environ.get("CODEX_HOME", Path.home()/".codex"))/"sessions"
def hook_result(source,path,bindings=None):
    session=(parse_claude_code_session if source=="claude-code" else parse_muse_session if source=="muse" else parse_codex_session)(path,bindings)
    return ParseResult(convs=[session["conv"]],msgs=session["msgs"],tools=session["tools"],attachs=session["attachs"],edits=session["edits"],edit_evidence=session["edit_evidence"],tool_lineage=session["tool_lineage"]) if session else ParseResult()
def enqueue_hook(source, payload):
    path,root=(Path(payload["transcript_path"]).expanduser().resolve() if payload.get("transcript_path") else _muse_session_path(hook_root(source).expanduser().resolve(),payload.get("session_id")) if source=="muse" else None),hook_root(source).expanduser().resolve()
    if source not in ("claude-code", "codex", "muse") or path is None or path.suffix != ".jsonl" or not path.is_relative_to(root): raise ValueError(f"Invalid {source} transcript path")
    st,key=path.stat(),gen_id("hook",f"{source}:{path}")
    if payload.get("hook_event_name", "Stop") not in ("Stop","SessionEnd","SubagentStop"):
    ...
    with operation_lock(HOOK_DIR/".drain.lock","hooks.drain",30 if block else 0,mandatory=block) as pulse:
        ...
        for n,(work,stamp) in enumerate(claims):
            ...
            try:
                ...
                (capture_provenance(edit_ids=r.provenance_edits,conversation_ids=r.provenance_conversations,source=f"{e['source']}.hook"),atomic_json(work,{**e,"snap":snap,"changed":sorted(changed)}),done.append((work,key,snap,changed)))
...
def sync(watch: bool = typer.Option(False, "-w"), interval: int = typer.Option(300, "-i"), claude_code: bool = True, codex: bool = True, full: bool = typer.Option(False, "--full", help="Re-parse/re-fetch all sources and reconcile all provenance"), verbose: bool = typer.Option(False, "-v", "--verbose"), local_only: bool = typer.Option(False, "--local-only", help="Import local agent sessions and configured exports without contacting web sources."), muse: bool = True):
    ...
    def plan_local(name, path, parser, bindings, sink, progress):
        ...
        if name in ("codex", "claude-code", "muse"):
    ...
    def run_sync(pulse):
        ...
        if paths := [Path(p).expanduser() for p in os.environ.get("CONVOS_IMPORT_PATHS", "").split(",") if p.strip()]: jobs+=start("imports") or [j for p in paths if (j:=plan_import(p))]
        for name,label,enabled,p,parser in (("claude-code","Claude Code",claude_code,Path(os.environ.get("CLAUDE_CONFIG_DIR",Path.home()/".claude"))/"projects",parse_claude_code),("codex","Codex",codex,Path(os.environ.get("CODEX_HOME",Path.home()/".codex")),parse_codex),("muse","Muse",muse,Path(os.environ.get("MUSE_HOME",Path.home()/".local/share/muse"))/"sessions",parse_muse)):
