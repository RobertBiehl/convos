...
CREATE OR REPLACE VIEW current_file_edits AS SELECT e.* FROM file_edits e WHERE NOT EXISTS(SELECT 1 FROM parser_edit_history h WHERE h.old_id=e.id);
...
def parser_tool_lineage(pairs,kind="tool_calls"): return sorted({(new['message_id'],old['id'],provenance_digest(parser_logical_row(kind,{k:json.dumps(v) if k in ROW_JSON_V1 else stamp if k=='created_at' else v for k,v in old.items()})),new['id'],provenance_digest(parser_logical_row(kind,new))) for old,new in pairs if old['id']!=new['id'] for stamp in {old['created_at'],old['created_at'].astimezone().replace(tzinfo=None) if old['created_at'] is not None and old['created_at'].tzinfo else old['created_at']}})
...
def prepare_edit_lineage(db,r):
    ...
    paths,routes=captured_edit_paths(db,ids),dict(db.execute('SELECT file_edit_id,route FROM provenance.file_edit_scopes WHERE file_edit_id IN (SELECT UNNEST(?))',[list(ids)]).fetchall())
    variants=lambda row:[{**row,'file_path':path} for path in {row['file_path'],*([paths[row['id']]] if row['id'] in paths and routes.get(row['id']) and os.path.normpath(row['file_path'])==routes[row['id']] else [])}]
    pairs=[(old,new) for original,current in r.edit_lineage for prior in variants(original) for old in (prior,dict(zip(prior,_history_row('file_edits',list(prior.values()),tuple(v for i,v in enumerate(prior.values()) if i not in (0,5)))))) for new in variants(current)]
    prepare_tool_lineage(db,r,'file_edits',parser_tool_lineage(pairs,'file_edits'))
def prepare_tool_lineage(db,r,kind="tool_calls",lineage=None):
    ...
    claims,bodies,received=(claims:=[(kind,source,source,'','active') for source in {v[1] for v in lineage}]),(bodies:=typed_logical_rows(db,claims)),{r[0] for r in db.execute("SELECT physical_row_id FROM remote.row_origins WHERE table_name=? AND physical_row_id IN (SELECT UNNEST(?))",[kind,[v[1] for v in claims]]).fetchall()}
    kept=[(mid,dict(zip(('old_id','old_hash','current_id','current_hash'),value))) for mid,*value in lineage if value[0] not in received and (body:=bodies.get((kind,value[0],value[0],'','active'))) is not None and provenance_digest(body)==value[1]]
    for m in r.msgs:
