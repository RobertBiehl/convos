...
def rebuild_parser_lineage(db,kinds=(('tool_calls','messages','tool'),('file_edits','messages','edit'),('messages','conversations','message')),retire=True):
    for kind,owner,label in kinds:
        ...
        rows=db.execute(f"SELECT c.id,coalesce(o.author_user_id,''),o.physical_row_id IS NULL FROM {owner} c LEFT JOIN remote.row_origins o ON o.table_name=? AND o.physical_row_id=c.id WHERE json_exists(c.metadata,'$.convos_{label}_lineage')",[owner]).fetchall()
...
def record_parser_replicas(db,items):
    for kind,owner in (('tool_calls','messages'),('file_edits','messages'),('messages','conversations')):
        ...
        records=[((r['id'] if native and r['id'] in natives else origins.get((p['author_user_id'],r['id']),r['id'] if native else remote_id(p['author_user_id'],owner,r['id'])),p['author_user_id'],native),(r['data'] or {}).get('metadata')) for r,p,pid,native,*maps in items if r['kind']==owner]
        (record_tool_lineage(db,[v for v,m in records],kind,{v[0]:m for v,m in records}),reconcile_tool_lineage(db,[v[0] for v,m in records],[r['id'] for r,p,pid,native,*maps in items if r['kind']==kind],kind))
