...
def reconcile_tool_lineage(db,messages=(),tools=(),kind="tool_calls"):
    ...
    for author,native in {(r[1],r[2]) for r in rows}:
        for at in range(0,len(selected:=[r for r in rows if (r[1],r[2])==(author,native)]),500):
            ...
            if changes:=[((*((bindings[r[3]],bindings[r[5]]) if r[3] in bodies and r[5] in bodies and matching_logical_row(bodies[r[3]],r[4]) is not None and matching_logical_row(bodies[r[5]],r[6]) is not None else (None,None)),r[0],author,r[3],r[4],r[5],r[6])) for r in page]: db.executemany(f"UPDATE parser_{label}_lineage SET old_id=?,new_id=? WHERE message_id=? AND author=? AND old_source=? AND old_hash=? AND new_source=? AND new_hash=?",changes)
