...
def _capture_provenance(path=None,edit_ids=None,conversation_ids=None,source="sync",blocked=()):
    ...
    with _core(path,True,purpose="provenance.plan") as core:
        queued,((eids, cids))=(queued:=core.execute("SELECT kind,entity FROM provenance.pending ORDER BY generation,kind,entity"+(" LIMIT 500" if targeted else "")).fetchall()),(sorted(set(eids)|{entity for kind,entity in queued if kind=='file_edits'}),sorted(set(cids)|{entity for kind,entity in queued if kind=='conversations'}))
...
def _apply_signed_edit_evidence(db,edits=(),tools=(),source=False):
    ...
    claims=[(kind,entity,entity,author,'active') for kind,entity,author,pid,expected in native]
...
def retire_parser_rows(db):
    for kind,label in (('tool_calls','tool'),('messages','message'),('conversations','conversation')):
        ...
        plans=(db.execute("CREATE OR REPLACE TEMP TABLE core_parser_retire(physical VARCHAR PRIMARY KEY,replacement VARCHAR,source VARCHAR,author VARCHAR,expected VARCHAR)"),db.execute(f"{fresh} UNION SELECT r.physical,r.replacement,r.source,r.author,r.content_hash FROM parser_retired_rows r JOIN {kind} old ON old.id=r.physical WHERE r.kind='{kind}'").fetchall())[1]
