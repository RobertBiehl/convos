...
def parser_tool_lineage(pairs): return sorted({(new['message_id'],old['id'],provenance_digest(parser_logical_row('tool_calls',{k:json.dumps(v) if k in ROW_JSON_V1 else stamp if k=='created_at' else v for k,v in old.items()})),new['id'],provenance_digest(parser_logical_row('tool_calls',new))) for old,new in pairs if old['id']!=new['id'] for stamp in {old['created_at'],old['created_at'].astimezone().replace(tzinfo=None) if old['created_at'] is not None and old['created_at'].tzinfo else old['created_at']}})
