...
| Pending | Ship one backed-up, resumable, bounded-memory identity and ingestion migration. Dedupe within author and provider transcript identity, never by root session alone and never across relay authors. Preserve safe filename-to-native aliases, re-read surviving raw inputs only after native identity bindings are installed, quarantine promptless startup stubs, and prove interruption recovery. |
